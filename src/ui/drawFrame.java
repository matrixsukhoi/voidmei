...
public class drawFrame extends WebFrame implements Runnable {
		...
		panel = new WebPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				if(pixIndex==5){
					...
					drawXY(g2d,50,50,dwidth,dheight,"爬升对比","时间","高度","s","m",xmin,xmax,ymin,ymax,xgap,ygap);
				...
				if(pixIndex==6){
					...
					drawXY(g2d,50,50,dwidth,dheight,"速度-高度曲线（FM文件隐藏面板数据）","速度","高度","km/h","m",xmin,xmax,ymin,ymax,xgap,ygap);
				...
				if(pixIndex==7){
					...
					drawXY(g2d,50,50,dwidth,dheight,"速度-滚转率曲线（解析自FM文件）","速度","滚转率","km/h","Deg/s",xmin,xmax,ymin,ymax,xgap,ygap);
	...
	public void run() {
		while (doit) {
			...
			// System.out.println("drawFrame执行了");
			repaint();
		}
	}
}