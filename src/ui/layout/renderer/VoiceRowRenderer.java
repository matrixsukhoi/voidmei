...
public class VoiceRowRenderer implements RowRenderer {
    ...
    public WebPanel render(RowConfig row, GroupConfig groupConfig, RenderContext context) {
        ...
        // 使用 VoicePackConfig 工具方法剥离前缀
        String voiceKey = VoicePackConfig.stripVoicePrefix(row.property);
        ...
        // 使用 VoicePackConfig 解析配置值
        String rawVal = null;
        if (row.property != null) {
            rawVal = context.getStringFromConfigService(row.property, row.getStr());
        }

        VoicePackConfig config = VoicePackConfig.parse(rawVal);
        combo.setSelectedItem(config.packName);
        enableSwitch.setSelected(config.enabled);

        // Listeners - 使用 VoicePackConfig 序列化配置值
        java.awt.event.ActionListener stateSaver = e -> {
            ...
            // 使用 VoicePackConfig 序列化
            String newVal = new VoicePackConfig(newPack, newEnabled).toConfigString();
        ...
        // Play Button - 使用 VoicePackConfig 工具方法
        ...
        btnPlay.addActionListener(e -> {
            ...
            String pKey = VoicePackConfig.stripVoicePrefix(row.property);
        ...
        // Validation logic - 使用 VoicePackConfig 工具方法
        Runnable validate = () -> {
            String validationPack = (String) combo.getSelectedItem();
            if (validationPack == null)
                validationPack = VoicePackConfig.DEFAULT_PACK;

            String vKey = VoicePackConfig.stripVoicePrefix(row.property);
        ...
        // Subscribe to update events - 使用 VoicePackConfig 解析配置
        if (row.property != null) {
            java.util.function.Consumer<Object> updateHandler = key -> {
                if (key instanceof String && ((String) key).equals(row.property)) {
                    ...
                    VoicePackConfig cfg = VoicePackConfig.parse(val);
                    ...
                    javax.swing.SwingUtilities.invokeLater(() -> {
                        ...
                        combo.setSelectedItem(cfg.packName);
                        enableSwitch.setSelected(cfg.enabled);
        ...
        java.util.function.Consumer<Object> refreshHandler = ignore -> {
            ...
            refreshedValidPacks.add(VoicePackConfig.DEFAULT_PACK);

            String rKey = VoicePackConfig.stripVoicePrefix(row.property);
