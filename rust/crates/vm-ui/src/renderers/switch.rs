...
//!
//! **D9 变更**: 原 iced view_row 与读链 (read_display) 已删 (渲染归 vm-webui
//! web 壳, 回显走整树 DTO), 本模块仅存写链 (apply)。
//!
//! 写回语义保真:
//! - SWITCH: writeBool (组字段 + 服务同步), 绑定失败回落 row.value (Java L64-66)。
//! - SWITCH_INV: 显示值取反落库 + row.value 存显示值 (Java L33-38)。
