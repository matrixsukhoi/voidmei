...
//!
//! **D9 变更**: 设置窗换 Tauri 2 web 壳 (vm-webui) — 表单渲染归 web 壳,
...
pub enum Message {
    ...
    /// vm-app dispatcher 在表单写链前拦截直接开窗, 不达本层)
...
impl MainFormState {
    ...
    /// panel 树快照只读访问 (vm-webui dto 序列化用)
    pub fn groups(&self) -> &[GroupConfig] {
        &self.groups
    }

    /// 配置服务直访 (vm-app dispatcher 的 import/reset 链用)
