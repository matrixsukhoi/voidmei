...
//!
//! - [`ZebraList`] — 斑马纹行渲染: 表头琥珀 #503C00 / 偶 #191919 / 奇 #282828,
//!   margin 2,6,2,6。
//! - [`BaseListOverlay`] — 轮询-脏检查-高度自适应状态机 (200ms 默认)。
//!
//! BaseOverlay.run() 的 while(doit)+sleep 线程循环在
...
//!
//! 行几何模型 (WebLabel + VerticalFlowLayout(0,0)):
...
/// 表头底色 new Color(80, 60, 0, alpha) — 深琥珀 #503C00
const HEADER_RGB: [u8; 3] = [80, 60, 0];
/// 偶数行 new Color(25, 25, 25, alpha) — #191919
const ZEBRA_EVEN_RGB: [u8; 3] = [25, 25, 25];
/// 奇数行 new Color(40, 40, 40, alpha) — #282828
const ZEBRA_ODD_RGB: [u8; 3] = [40, 40, 40];
/// dataPanel 底色 new Color(20, 20, 20, alpha) — #141414,
/// 行未覆盖区域 (行数不足窗口高 / ±2px 高度容差带 / 空列表) 的兜底色
const PANEL_BG_RGB: [u8; 3] = [20, 20, 20];
/// label.setForeground(Color.WHITE) — 行文本恒白且不透明
const TEXT_COLOR: [u8; 4] = [255, 255, 255, 255];

/// label.setMargin(2, 6, 2, 6) 上下边距 (行高组成部分)
...
pub struct BaseListOverlay {
    /// stop flag (volatile doit; stop() 置 false)
    pub doit: bool,
    /// 行背景 alpha (默认 180; setAlpha 可改)
    pub alpha: u8,
    /// 预览模式 (initPreview 置 true — 免 isVisibleNow 门控)
    pub is_preview: bool,
    /// 窗口宽 (init 公式)
    pub width: i32,
    /// 当前窗口高 (adjustPosition 维护, :282 setSize 目标值)
    pub height: i32,
    /// init 时的 fontSize 字段 (派生量留档)
    pub font_size: i32,
    /// Application.logicalHeight 快照 (高度 clamp 上限来源, :275)
    pub logical_height: i32,
    /// getRefreshInterval() 默认 200ms
    pub refresh_interval_ms: u64,
    /// isVisibleNow() 默认 true (子类可覆盖为开关态)
    ...
    row_height: i32,
    /// lastData (脏检查基准; null = 尚无数据)
    last_data: Option<Vec<String>>,
    /// pluggable renderer (默认 ZebraListRenderer)
    pub zebra: ZebraList,
}
...
impl BaseListOverlay {
        ...
        F: FnOnce() -> Option<Vec<String>>,
    {
        ...
        if self.is_preview || self.visible_now {
            ...
            if dirty {
                self.adjust_position(); // updateUI → adjustPosition
            }
            dirty
        } else {
            self.window_visible = false; // PORT: else 分支 setVisible(false)
            false
        }
    }
    ...
    /// updateUI 的渲染段 (渲染线程上的 renderer.render):
