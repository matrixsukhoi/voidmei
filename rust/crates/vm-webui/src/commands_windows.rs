...
//! | [`flight_record_data`] | `ui/overlay/DrawFrame.java` | FlightAnalyzer 曲线族快照 (init(c,A) 修正后的 paintAction 读取子集) |
...
//! 唯一的共享态是飞行记录快照 ([`FLIGHT_RECORD_SNAPSHOT`]): 发布入口
//! [`publish_flight_record`] 预定由 vm-app `close_flight_log` 在关日志时调用,
//! 命令线程凭快照取数 (跨线程快照模式, flight_analyzer 集成合同的消费端)。
//! TODO(port): 该接线尚未落地 — vm-app app_shell.rs 的 close_flight_log 仍标
//! "DrawFrame 段 D8 豁免" 未调 publish_flight_record (接线改 vm-app 文件, 超出
//! 本批 vm-webui 范围), 归阶段④随飞行记录前端窗口开窗事件一并补; 接线前
//! flight_record_data 在生产运行恒无快照可读 (恒 Err), 快照仅测试发布过。
//!
...
use vm_core::flight_analyzer::FlightAnalyzer;
...
use crate::dto::{
    ComparisonDataDto, ComparisonRowDto, FlightPageDto, FlightRecordDataDto, FlightSeriesDto,
    InflectionPointDto, PowerCurveDataDto, PowerCurveDto,
};
...
// =====================================================================
// 3. 飞行记录曲线窗口数据 (DrawFrame.java)
// =====================================================================

// 对位 Java `Service.ENGINE_TYPE_PROP / ENGINE_TYPE_TURBOPROP`
// (vm-data service_fields.rs 的值域镜像 — vm-webui 不依赖 vm-data, DrawFrame
// 页1 判型分支 DrawFrame.java:539-540 只用这两个值)。
// 备案: vm-data 侧值变更时此处静默失同步 — 引擎类型是稳定枚举 (0/1/2 自
// Java 原版未动过), 变更概率极低; 若失同步仅影响页1 功率/推力页选择
const ENGINE_TYPE_PROP: i32 = 0;
const ENGINE_TYPE_TURBOPROP: i32 = 2;

/// DrawFrame 消费面的飞行记录快照 (跨线程快照模式)。
/// Java: Controller.closepad/onAircraftChanged 在 Log.close() 前以 `Log.fA` 弹
/// DrawFrame (Controller.java:323-325/405-407) — AppShell 主线程 (!Send) 持有的
/// FlightAnalyzer 经 [`publish_flight_record`] 发布只读 DTO, web 命令线程凭
/// 快照取数, 不回触主线程 (flight_analyzer 集成合同的消费端)。
///
/// 备案 (LIFETIMES §1.3 全局静态例外): 非 OnceLock<RwLock> 单例常态 — 作为 D8
/// "AppShell !Send 恒留主线程" 约束下的跨线程快照桥接, 发布端唯一 (主线程
/// publish) / 读取端无锁快照 (命令线程 clone Arc); 若阶段④出现多写者再收敛进
/// AppShell 拥有的 slot。覆盖式常驻 vs Java "窗口关闭即丢" 的语义差, 由阶段④
/// 前端窗口生命周期裁决 (届时定是否需要 clear 钩子)。
static FLIGHT_RECORD_SNAPSHOT: RwLock<Option<Arc<FlightRecordDataDto>>> = RwLock::new(None);

/// 发布飞行记录快照 (重复发布 = 覆盖, 对位 Java 每次关日志 new DrawFrame 替换
/// 旧窗)。
///
/// TODO(port): 生产接线未落地 — 预定调用点是 vm-app app_shell.rs 的
/// close_flight_log (Java closepad/onAircraftChanged 的 showdrawFrame 段,
/// `Log.close()` 前取 `Log.fA` 发布); 该文件超出本批范围 (仅 vm-webui), 接线
/// 归阶段④, 接线前快照仅测试发布。
///
/// 防护两层 (对位 Java): ① 调用点守卫 `curaltStage - initaltStage >= 1`
/// (Controller.java:323/405 — 不满足根本不弹窗, 此处直接不发布); ② build 的
/// 越界 panic 兜底 — Java DrawFrame.init 的 AIOOBE 逃逸到 Service.java:1850
/// 顶层 catch(Exception) 被吞 (无窗 + error 日志), Rust 主线程 pump 无此兜底,
/// 此处 catch_unwind 复刻 "崩发布不崩应用" (panic 不覆盖既有快照 = Java 旧窗
/// 保留)。
pub fn publish_flight_record(fa: &FlightAnalyzer) {
    // ① Java 调用点守卫: 爬升档数 ≥1 才弹 DrawFrame
    if fa.curalt_stage - fa.initalt_stage < 1 {
        return;
    }
    // ② 畸形 fa (如 initaltStage 贴近数组尾使 [init+2] 越界) 的 panic 兜底
    let built = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        Arc::new(build_flight_record(fa))
    }));
    match built {
        Ok(dto) => {
            *FLIGHT_RECORD_SNAPSHOT
                .write()
                .unwrap_or_else(|e| e.into_inner()) = Some(dto);
        }
        Err(_) => {
            // 对位 Service.java:1852 顶层 catch 的 error 日志 (吞异常继续)
            logger::error(
                "DrawFrame",
                "飞行记录快照构造异常 (Java ArrayIndexOutOfBoundsException 对位, 已吞)",
            );
        }
    }
}

/// 读取当前快照 (命令线程; None = 尚无飞行记录)
pub fn current_flight_record() -> Option<Arc<FlightRecordDataDto>> {
    FLIGHT_RECORD_SNAPSHOT
        .read()
...
/// Java drawCoordinates 连线段的跳变修正 (审查 A2 补齐): double[] 版
/// DrawFrame.java:392-394 / int[] 版 :480-486, 两版循环体相同 —
/// `|X[ii]-X[ii+1]|>100 时 X[ii+1]=X[ii+2]` 以替换值绘制该处线段并就地改数组
/// (后续轮次的比较读替换后的值)。跳变真实可触发: init 首档线性外推点 /
/// 增压器换级 / 油门突变使 power/thrust 相邻档差>100。X[ii+2] 可越出显示切片
/// 一位 (ii ≤ curalt-2 → ii+2 ≤ curalt), 故在完整数组上跑; 越界 panic 交
/// [`publish_flight_record`] 的 catch_unwind 兜底 (对位 Java AIOOBE 被
/// Service 顶层 catch 吞)。
fn fix_draw_jump_f64(x: &mut [f64], init: usize, curalt: usize) {
    // Java for (ii=initalt; ii<curalt-1; ii++) — 每轮就地改值, 顺序敏感不可并行
    let mut ii = init;
    while ii + 1 < curalt {
        if (x[ii] - x[ii + 1]).abs() > 100.0 {
            x[ii + 1] = x[ii + 2];
        }
        ii += 1;
    }
...
/// int[] 版同款: Math.abs(int) 的回绕面 — 差值溢出回绕 + abs(MIN)=MIN 仍为负 →
/// wrapping 复刻 (溢出时判定恒 false, 与 Java 一致)
fn fix_draw_jump_i32(x: &mut [i32], init: usize, curalt: usize) {
    let mut ii = init;
    while ii + 1 < curalt {
        if x[ii].wrapping_sub(x[ii + 1]).wrapping_abs() > 100 {
            x[ii + 1] = x[ii + 2];
        }
        ii += 1;
    }
...
/// Java `DrawFrame.init(Controller, FlightAnalyzer)` 的数据面 (DrawFrame.java:490-556
/// + paintAction 各页实际读取的数组): init 的数组修正 (首档线性外推) + 连线跳变
/// 修正 (drawCoordinates, 上述两助手) + fY 高度轴 + getdata(fA.type) 的 FM 载入
/// (page6), 输出 7 页 (pixIndex 0/1/2/3/5/6/7; 4 无页面)。
///
/// Java 在共享 fA 上就地修正 (init 后 FlightLog 已 close, 无人再读); 此处快照副本
/// 上修正, 可观察行为等价。
pub fn build_flight_record(fa: &FlightAnalyzer) -> FlightRecordDataDto {
    // 特殊处理：接收 FlightAnalyzer 数据用于绘制曲线
    // 注意：Controller 参数保留是为了与 Controller.showdrawFrame() 的调用签名兼容
    // (原注释保留 — Controller 面在 web 窗口域无数据消费)
    let mut time = fa.time.clone();
    let mut power = fa.power.clone();
    let mut thrust = fa.thrust.clone();
    let mut eff = fa.eff.clone();
    let mut sep = fa.sep.clone();
    let init = fa.initalt_stage as usize;

    // fA.time[initaltStage] = 0; 其余四族 [init] = [init+1] + [init+1] - [init+2]
    // (首档值由后两档线性外推)。power/thrust/eff 是 int[] — Java int 算术静默
    // 回绕 → wrapping (§2.2)
    time[init] = 0.0;
    power[init] = power[init + 1]
        .wrapping_add(power[init + 1])
        .wrapping_sub(power[init + 2]);
    thrust[init] = thrust[init + 1]
        .wrapping_add(thrust[init + 1])
        .wrapping_sub(thrust[init + 2]);
    eff[init] = eff[init + 1]
        .wrapping_add(eff[init + 1])
        .wrapping_sub(eff[init + 2]);
    // sep 是 double[] — 普通 f64 算术
    sep[init] = sep[init + 1] + sep[init + 1] - sep[init + 2];

    // 连线跳变修正 (审查 A2): init 修正后、切片前对五族数组完整副本就地跑
    // Java drawCoordinates 连线循环的同款修正 — DTO 的 x 序列即 Java 首绘实际
    // 落屏的折线 (替换值), 跳变处与 Java 可观察行为一致
    fix_draw_jump_f64(&mut time, init, fa.curalt_stage as usize);
    fix_draw_jump_i32(&mut power, init, fa.curalt_stage as usize);
    fix_draw_jump_i32(&mut thrust, init, fa.curalt_stage as usize);
    fix_draw_jump_i32(&mut eff, init, fa.curalt_stage as usize);
    fix_draw_jump_f64(&mut sep, init, fa.curalt_stage as usize);

    // fY = new double[curaltStage - initaltStage]: 高度轴, 每档 100m
    // Java: int xk = initaltStage; fY[i] = xk * 100 (int 乘后加宽) — 档数 ≤256 无溢出
    let n = (fa.curalt_stage - fa.initalt_stage) as usize;
    let f_y: Vec<f64> = (0..n)
        .map(|i| ((fa.initalt_stage + i as i32) * 100) as f64)
        .collect();

    // getdata(fA.type) — P5 收编: 按机型名显式加载（飞行结束曲线窗口，独立于
    // FMManager 当前机型缓存）。非 READY（MISSING/CORRUPT）句柄 blkx 为 null，
    // 置 None 交给 page6 判空守卫
    let handle = fm_loader::load(fa.r#type.as_deref());
    let blkx = if handle.has_fm() { handle.blkx } else { None };

    let lang = Lang::init_lang();

    // drawCoordinates 族 (pixIndex 0-3): X 轴 = 数据, Y 轴 = 高度 (fY);
    // 各页取 [initalt, curalt) 切片 (Java 绘点循环 ii: initalt..=curalt-1)
    let slice = |arr: &[f64]| -> Vec<f64> { arr[init..init + n].to_vec() };
    let series = |x: Vec<f64>, y: Vec<f64>| FlightSeriesDto {
        x,
        y,
        color: "black".to_string(), // Java drawCoordinates/drawPoint 的 Color(0,0,0,250)
        legend: None,
    };
    let mut pages: Vec<FlightPageDto> = Vec::new();

    // pixIndex == 0: 时间-高度曲线
    pages.push(FlightPageDto {
        index: 0,
        title: lang.d_f_title1.to_string(),
        x_name: lang.d_f_title1_x.to_string(),
        y_name: lang.d_f_title1_y.to_string(),
        x_unit: "s".to_string(),
        y_unit: "m".to_string(),
        series: vec![series(slice(&time), f_y.clone())],
        error_text: None,
    });

    // pixIndex == 1: 活塞/涡桨画功率, 其余画推力 (DrawFrame.java:539-549)
    if fa.engine_type == ENGINE_TYPE_PROP || fa.engine_type == ENGINE_TYPE_TURBOPROP {
        pages.push(FlightPageDto {
            index: 1,
            title: lang.d_f_title2.to_string(),
            x_name: lang.d_f_title2_x.to_string(),
            y_name: lang.d_f_title2_y.to_string(),
            x_unit: "hp".to_string(),
            y_unit: "m".to_string(),
            series: vec![series(
                slice(&power.iter().map(|&v| v as f64).collect::<Vec<f64>>()),
                f_y.clone(),
            )],
            error_text: None,
        });
        ...
        pages.push(FlightPageDto {
            index: 1,
            title: lang.d_f_title3.to_string(),
            x_name: lang.d_f_title3_x.to_string(),
            y_name: lang.d_f_title3_y.to_string(),
            x_unit: "kg".to_string(),
            y_unit: "m".to_string(),
            series: vec![series(
                slice(&thrust.iter().map(|&v| v as f64).collect::<Vec<f64>>()),
                f_y.clone(),
            )],
            error_text: None,
        });
    ...
    pages.push(FlightPageDto {
        index: 2,
        title: lang.d_f_title4.to_string(),
        x_name: lang.d_f_title4_x.to_string(),
        y_name: lang.d_f_title4_y.to_string(),
        x_unit: "hp".to_string(),
        y_unit: "m".to_string(),
        series: vec![series(
            slice(&eff.iter().map(|&v| v as f64).collect::<Vec<f64>>()),
            f_y.clone(),
        )],
        error_text: None,
    });
    ...
    pages.push(FlightPageDto {
        index: 3,
        title: lang.d_f_title5.to_string(),
        x_name: lang.d_f_title5_x.to_string(),
        y_name: lang.d_f_title5_y.to_string(),
        x_unit: "m/s".to_string(),
        y_unit: "m".to_string(),
        series: vec![series(slice(&sep), f_y)],
        error_text: None,
    });

    // pixIndex == 5: 示空速-法向过载曲线 (EM 图 turn_load 族)
    {
        let num = fa.get_no_zeros_num_f64(&fa.turn_load) as usize;
        let mut iasx = vec![0.0f64; num];
        let mut gy = vec![0.0f64; num];
        let mut gseploss = vec![0.0f64; num]; // Java 计算但未绘 (removeLoadZeroes 三参), 保形
        fa.remove_load_zeroes(&mut iasx, &mut gy, &mut gseploss);
        pages.push(FlightPageDto {
            index: 5,
            title: "示空速-法向过载曲线".to_string(),
            x_name: "示空速".to_string(),
            y_name: "法向过载".to_string(),
            x_unit: "km/h".to_string(),
            y_unit: "G".to_string(),
            series: vec![FlightSeriesDto {
                x: iasx,
                y: gy,
                color: "black".to_string(),
                legend: Some("法向过载".to_string()),
            }],
            error_text: None,
        });
    }

    // pixIndex == 6: 速度-高度曲线（FM文件隐藏面板数据, Blkx.loc1/loc2）
    {
        // P5: FMLoader 收编后加载失败时 Blkx 为 null（旧代码为 valid=false 的半成品
        // 对象, loc1/loc2 同样是 null 会 NPE）—— 判空跳过本页绘制
        // (Java 的 xmax 双取 findMax(loc1.x) 笔误属像素域, 未随数据面翻译)
        let (series6, error_text) = match blkx.as_ref() {
            Some(b) if b.loc1.is_some() && b.loc2.is_some() => {
                ...
                (
                    vec![
                        FlightSeriesDto {
                            x: loc1.x.clone(),
                            y: loc1.y.clone(),
                            color: "red".to_string(), // Java Color.red — WEP 速度
                            legend: Some("WEP速度".to_string()),
                        },
                        FlightSeriesDto {
                            x: loc2.x.clone(),
                            y: loc2.y.clone(),
                            color: "blue".to_string(), // Java Color.blue — 100% 油门
                            legend: Some("100%油门速度".to_string()),
                        },
                    ],
                    None,
                )
            ...
            _ => (
                Vec::new(),
                Some("FM数据未加载".to_string()), // Java g2d.drawString 提示
            ),
        };
        pages.push(FlightPageDto {
            index: 6,
            title: "速度-高度曲线（FM文件隐藏面板数据）".to_string(),
            x_name: "速度".to_string(),
            y_name: "高度".to_string(),
            x_unit: "km/h".to_string(),
            y_unit: "m".to_string(),
    ...
    // pixIndex == 7: 示速度-滚转率曲线 (EM 图 roll_rate 族)
    {
        let num = fa.get_no_zeros_num_i32(&fa.roll_rate) as usize;
        let mut iasx = vec![0.0f64; num];
        let mut wx = vec![0.0f64; num];
        fa.remove_roll_rates_zeroes(&mut iasx, &mut wx);
        pages.push(FlightPageDto {
            index: 7,
            title: "示速度-滚转率曲线".to_string(),
            x_name: "示速度".to_string(),
            y_name: "滚转率".to_string(),
            x_unit: "km/h".to_string(),
            y_unit: "Deg/s".to_string(),
            series: vec![FlightSeriesDto {
                x: iasx,
                y: wx,
                color: "black".to_string(),
                legend: Some("滚转率".to_string()),
            }],
            error_text: None,
        });
    }
    ...
    FlightRecordDataDto {
        // Java setTitle(fA.type + Lang.dFTitleHZ) — null 引用拼接为 "null"
        window_title: format!("{}{}", fa.r#type.as_deref().unwrap_or("null"), lang.d_f_title_hz),
        plane_type: fa.r#type.clone(),
        engine_type: fa.engine_type,
        initalt_stage: fa.initalt_stage,
        curalt_stage: fa.curalt_stage,
        pages,
        fm_loaded: blkx.is_some(),
    }
}
...
/// 飞行记录曲线数据 (DrawFrame 消费面; AppShell 关日志时发布的快照)。
/// 快照读 + 序列化为轻量面 (无 FM 解析)
#[tauri::command]
pub async fn flight_record_data() -> Result<serde_json::Value, String> {
    match current_flight_record() {
        Some(dto) => to_json(&*dto),
        None => Err("暂无飞行记录数据 (需先完成一次飞行日志记录)".to_string()),
    }
}
...
mod tests {
    ...
    // ---- 飞行记录 (FlightAnalyzer 快照构造; FMLoader 腿依赖真机 data/) ----

    fn sample_analyzer() -> FlightAnalyzer {
        // FlightAnalyzer 构造面: Default 全零 + 公开字段注入 (Java public 字段直填)
        let mut fa = FlightAnalyzer::default();
        fa.r#type = Some("spitfire_f24".to_string());
        fa.engine_type = 0; // 活塞 (Service.ENGINE_TYPE_PROP)
        fa.initalt_stage = 2;
        fa.curalt_stage = 5;
        for (i, t) in [10.0f64, 20.0, 30.0, 40.0, 50.0, 60.0].into_iter().enumerate() {
            fa.time[i] = t;
        }
        for (i, p) in [0i32, 0, 1500, 1400, 1300, 1200].into_iter().enumerate() {
            fa.power[i] = p;
            fa.thrust[i] = p;
            fa.eff[i] = p / 2;
        }
        for (i, s) in [0.0f64, 0.0, 15.0, 14.0, 13.0, 12.0].into_iter().enumerate() {
            fa.sep[i] = s;
        }
        fa
    }
    ...
    #[test]
    fn 飞行记录_init修正与页族() {
        let data_root_ok = ensure_real_data();
        if !data_root_ok {
            println!("SKIP: 真机 data/ 不存在 (page6 FM 腿不可验, 其余断言仍全量执行)");
        }
        let dto = build_flight_record(&sample_analyzer());

        // init 修正: time[init]=0; power/thrust/eff/sep[init] = 2*[init+1] - [init+2]
        let p0 = &dto.pages.iter().find(|p| p.index == 0).unwrap().series[0];
        assert_eq!(p0.x[0], 0.0, "time[initalt] 修正为 0");
        assert_eq!(p0.x.len(), 3, "切片长度 = curalt - initalt");
        // 高度轴: init=2 → 200/300/400
        assert_eq!(p0.y, vec![200.0, 300.0, 400.0]);
        // power 页 (活塞): x[0] = 2*1400 - 1300 = 1500
        let p1 = &dto.pages.iter().find(|p| p.index == 1).unwrap().series[0];
        assert_eq!(p1.x[0], 1500.0);
        assert_eq!(p1.x, vec![1500.0, 1400.0, 1300.0]);
        // eff 页: int 外推 2*700-650 = 750
        let p2 = &dto.pages.iter().find(|p| p.index == 2).unwrap().series[0];
        assert_eq!(p2.x[0], 750.0);
        // sep 页 (double): 2*14-13 = 15
        let p3 = &dto.pages.iter().find(|p| p.index == 3).unwrap().series[0];
        assert_eq!(p3.x[0], 15.0);

        // 页族: 0/1/2/3/5/6/7 (页 4 无)
        let idx: Vec<i32> = dto.pages.iter().map(|p| p.index).collect();
        assert_eq!(idx, vec![0, 1, 2, 3, 5, 6, 7]);
        // EM 族全零 → 空序列 (num=0 数组)
        assert!(dto.pages.iter().find(|p| p.index == 5).unwrap().series[0].x.is_empty());
        assert!(dto.pages.iter().find(|p| p.index == 7).unwrap().series[0].x.is_empty());
        // 标题与窗口名
        assert_eq!(dto.window_title, format!("spitfire_f24{}", Lang::init_lang().d_f_title_hz));
        assert_eq!(dto.initalt_stage, 2);
        assert_eq!(dto.curalt_stage, 5);

        if data_root_ok {
            // page6: FMLoader 载入 spitfire_f24 成功; 该机型 PASSPORT 无速度曲线
            // (fm 内无 maxSpeedWep 标签) → loc1/loc2 为空曲线, 序列在位但无点
            // (Java 同形态: 空白页, 无报错)
            assert!(dto.fm_loaded, "真机 data 在位时 FM 应载入");
            let p6 = dto.pages.iter().find(|p| p.index == 6).unwrap();
            assert_eq!(p6.error_text, None);
            assert_eq!(p6.series.len(), 2);
            assert_eq!(p6.series[0].color, "red");
            assert_eq!(p6.series[0].legend.as_deref(), Some("WEP速度"));
            assert_eq!(p6.series[1].color, "blue");
            assert_eq!(p6.series[1].legend.as_deref(), Some("100%油门速度"));
            assert!(p6.series[0].x.is_empty(), "spitfire_f24 无速度曲线数据");

            // a_129_a 的 PASSPORT 携带速度曲线 → loc1/loc2 非空 (真数据腿)
            let mut fa = sample_analyzer();
            fa.r#type = Some("a_129_a".to_string());
            let dto2 = build_flight_record(&fa);
            assert!(dto2.fm_loaded);
            let p6b = dto2.pages.iter().find(|p| p.index == 6).unwrap();
            assert_eq!(p6b.error_text, None);
            assert!(!p6b.series[0].x.is_empty(), "a_129_a loc1 曲线非空");
            assert_eq!(p6b.series[0].x.len(), p6b.series[0].y.len());
            assert!(!p6b.series[1].x.is_empty(), "a_129_a loc2 曲线非空");
        } else {
            assert!(!dto.fm_loaded);
            let p6 = dto.pages.iter().find(|p| p.index == 6).unwrap();
            assert_eq!(p6.error_text.as_deref(), Some("FM数据未加载"));
        }
    }
    ...
    fn 飞行记录_喷气机画推力页() {
        if !ensure_real_data() {
            println!("SKIP: 真机 data/ 不存在 (喷气机页选择与 data 无关, 仍执行断言)");
        }
        let mut fa = sample_analyzer();
        fa.engine_type = 1; // 喷气 (Service.ENGINE_TYPE_JET)
        let dto = build_flight_record(&fa);
        let p1 = dto.pages.iter().find(|p| p.index == 1).unwrap();
        // 推力页: x = thrust 切片 (2*1400-1300 = 1500)
        assert_eq!(p1.series[0].x, vec![1500.0, 1400.0, 1300.0]);
        assert_ne!(p1.title, Lang::init_lang().d_f_title2, "喷气机不应画功率页");
    }

    /// 审查 A2: drawCoordinates 连线跳变修正 — |X[ii]-X[ii+1]|>100 时以
    /// X[ii+2] 顶替绘制并就地改数组 (后续轮次比较读替换值)
    #[test]
    fn 飞行记录_连线跳变修正对位java() {
        if !ensure_real_data() {
            println!("SKIP: 真机 data/ 不存在 (页族构造与 data 无关, 仍执行断言)");
        }
        let mut fa = sample_analyzer();
        // power (int[] 版): init 外推 [2]=2*1400-1250=1550, |1550-1400|=150>100
        // → [3] 被顶替为 [4]=1250; 下一轮 |1250-1250|=0 不再触发
        for (i, p) in [0i32, 0, 0, 1400, 1250, 1200].into_iter().enumerate() {
            fa.power[i] = p;
        }
        // sep (double[] 版): [2]=2*200-50=350, |350-200|=150>100 → [3] 顶替为 [4]=50
        for (i, s) in [0.0f64, 0.0, 0.0, 200.0, 50.0, 0.0].into_iter().enumerate() {
            fa.sep[i] = s;
        }
        let dto = build_flight_record(&fa);
        let p1 = &dto.pages.iter().find(|p| p.index == 1).unwrap().series[0];
        assert_eq!(
            p1.x,
            vec![1550.0, 1250.0, 1250.0],
            "int[] 跳变修正: 顶替值参与连线且就地生效"
        );
        let p3 = &dto.pages.iter().find(|p| p.index == 3).unwrap().series[0];
        assert_eq!(p3.x, vec![350.0, 50.0, 50.0], "double[] 跳变修正同款");
        // time[init]=0 与次档差 40 <100 不触发 (无跳变页保持原值)
        let p0 = &dto.pages.iter().find(|p| p.index == 0).unwrap().series[0];
        assert_eq!(p0.x, vec![0.0, 40.0, 50.0]);
    }

    #[test]
    fn 飞行记录_滚转率曲线非零出数() {
        if !ensure_real_data() {
            println!("SKIP: 真机 data/ 不存在");
        }
        let mut fa = sample_analyzer();
        // 滚转率 EM: 非零档位 → iasx = i*10, wx 三点滑动均值
        fa.roll_rate[10] = 90;
        fa.roll_rate[11] = 100;
        fa.roll_rate[12] = 110;
        let dto = build_flight_record(&fa);
        let p7 = dto.pages.iter().find(|p| p.index == 7).unwrap();
        let num = fa.get_no_zeros_num_i32(&fa.roll_rate) as usize;
        assert_eq!(num, 3);
        assert_eq!(p7.series[0].x.len(), num);
        // x = i*10 (100/110/120), y = (前+本+后)/3 — 边界档位邻档为 0
        // (Java removeZeroes 从 i=0 起遍历, oy[10]!=0 时读 oy[9]/oy[11])
        let avg3 = |a: f64, b: f64, c: f64| (a + b + c) / 3.0;
        assert_eq!(p7.series[0].x, vec![100.0, 110.0, 120.0]);
        assert_eq!(
            p7.series[0].y,
            vec![avg3(0.0, 90.0, 100.0), avg3(90.0, 100.0, 110.0), avg3(100.0, 110.0, 0.0)]
        );
    }

    /// FLIGHT_RECORD_SNAPSHOT 是全局静态, 涉及 publish 的测试用此锁串行
    /// (cargo test 默认并行, 防跨测试覆盖)
    static SNAP_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    #[test]
    fn 飞行记录_快照发布与读取() {
        let _g = SNAP_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // 跨线程快照模式: 发布 → 读取 → 覆盖 (对位每次关日志弹新窗)
        publish_flight_record(&sample_analyzer());
        let snap = current_flight_record().expect("发布后应可读取");
        assert_eq!(snap.plane_type.as_deref(), Some("spitfire_f24"));
        assert_eq!(snap.pages.len(), 7);
        // 覆盖发布 (换机)
        let mut fa2 = sample_analyzer();
        fa2.r#type = Some("another-plane".to_string());
        fa2.curalt_stage = 4;
        publish_flight_record(&fa2);
        let snap2 = current_flight_record().expect("覆盖发布后应可读取");
        assert_eq!(snap2.plane_type.as_deref(), Some("another-plane"));
        assert_eq!(snap2.pages[0].series[0].x.len(), 2, "curalt-initalt = 2");
    }

    #[test]
    fn 飞行记录_发布守卫与panic兜底() {
        let _g = SNAP_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // 基准发布
        publish_flight_record(&sample_analyzer());

        // ① 守卫: curalt - initalt < 1 不发布 (Java Controller.java:323/405 调用点
        // 条件 — 不满足根本不 new DrawFrame), 既有快照不被覆盖
        let mut guard = sample_analyzer();
        guard.r#type = Some("guard-case".to_string());
        guard.curalt_stage = guard.initalt_stage; // 爬升档数 0
        publish_flight_record(&guard);
        let snap = current_flight_record().expect("守卫不应清空既有快照");
        assert_eq!(snap.plane_type.as_deref(), Some("spitfire_f24"), "档数不足不覆盖");

        // ② panic 兜底: initalt=255 使 build 的 power[init+1] 越界 (数组长 256) —
        // 对位 Java DrawFrame.init 的 AIOOBE 被 Service.java:1850 顶层 catch 吞
        // (无窗 + error 日志, 旧窗保留)。静音 hook: catch_unwind 的 panic 噪音
        // 与断言无关
        let mut oob = sample_analyzer();
        oob.r#type = Some("oob-case".to_string());
        oob.initalt_stage = vm_core::flight_analyzer::MAX_ALT_STAGE - 1;
        oob.curalt_stage = vm_core::flight_analyzer::MAX_ALT_STAGE; // 差 1 过守卫
        let prev_hook = std::panic::take_hook();
        std::panic::set_hook(Box::new(|_| {}));
        publish_flight_record(&oob);
        std::panic::set_hook(prev_hook);
        let snap2 = current_flight_record().expect("panic 兜底不应清空既有快照");
        assert_eq!(snap2.plane_type.as_deref(), Some("spitfire_f24"), "panic 不覆盖 (Java 旧窗保留)");
    }
