//! 公式系统管理器 (波8 自 mod.rs 抽出, mod.rs 回归纯声明 — 对齐全库域惯例):
//! 持当前编译集 (原子换 Arc 热更新) + 状态原语仓 + :target 统一解析。
...
pub fn target_value(
    var: &TargetVar,
    mult: f64,
    s: &dyn super::registry::FormulaView,
) -> Option<f64> {
    match var {
        TargetVar::Var(vid) => {
...
impl FormulaManager {
    ...
    pub fn eval_frame(
        &self,
        raw: &super::registry::RawInputs,
        session: &super::registry::SessionInputs,
        meta: &MetaInputs,
        now_ms: u64,
