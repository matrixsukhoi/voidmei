...
impl FMManager {
	...
	pub fn invalidate(&self, name: Option<&str>) {
		...
		self.inner.negative_cache.lock().expect(LOCK_MSG).remove(&norm);
		self.inner.last_attempt_ms.lock().expect(LOCK_MSG).remove(&norm);
	}
	...
	fn submit_load(&self, target_name: &str) {
		...
		let (epoch, tx) = {
			...
			(self.inner.loader_epoch.load(Ordering::SeqCst), loader.clone())
		};
		...
		if let Err(reject) = tx.send(job) {
			...
			logger::error(
				"FMManager",
				&format!("FM-Loader 任务提交失败: {reject:?}"),
			);
		}
	}
...
mod tests {
	...
	fn real_data_identify_chain() {
		...
		if !real_fm_dir.join("spitfire_f24.blkx").exists() {
		...
		for root in ROOTS {
			...
			std::fs::copy(real_fm_dir.join("spitfire_f24.blkx"), dst.join("spitfire_f24.blkx"))
				.unwrap();
			std::fs::copy(
				real_fm_dir.join("fm/spitfire_f24.blkx"),
				dst.join("fm/spitfire_f24.blkx"),
			)
			.unwrap();
		}
