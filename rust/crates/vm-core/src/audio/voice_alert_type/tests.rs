...
fn test_alert_type_keys() {
    ...
    assert_eq!(VoiceAlertType::WarnGear.get_key(), "warn_gear", "WARN_GEAR key");
    assert_eq!(VoiceAlertType::FailEngine.get_key(), "fail_engine", "FAIL_ENGINE key");
    ...
    assert_eq!(VoiceAlertType::from_key(Some("nonexistent")), None, "fromKey nonexistent");
}

/// 对应 Java testAlertTypeCooldowns
#[test]
fn test_alert_type_cooldowns() {
    assert_eq!(VoiceAlertType::AoaCrit.get_cooldown_seconds(), 1, "AOA_CRIT cooldown");
    assert_eq!(VoiceAlertType::AoaHigh.get_cooldown_seconds(), 8, "AOA_HIGH cooldown");
    assert_eq!(
    ...
    // 验证毫秒转换
    assert_eq!(VoiceAlertType::AoaCrit.get_cooldown_ms(), 1000i64, "AOA_CRIT cooldown ms");
    assert_eq!(
