...
*   **Service Layer (Publisher)**: 它是唯一的真理来源 (Source of Truth)。它以可配置频率（默认 20Hz）向总线"广播"飞行数据的快照。它完全不知道（也不关心）有多少个 Overlay 在监听，也不关心它们如何显示。
...
1.  Service 线程计算完一帧数据后，将其打包成一个 **不可变 (Immutable) 的 EventPayload / HUDData**。
2.  这个快照随事件发送给 UI。
3.  UI 线程读取这个快照进行渲染。
...
*   **位置**: `src/ui/replica/ReplicaBuilder.java` API, `src/prog/event/EventPayload.java`
*   **实现**: 提供 `createSwitchItem`, `createSpinnerItem` 等静态方法，以及 `EventPayload.builder()` 建造者模式。
*   **架构价值**: 封装了复杂的组件初始化逻辑，让业务代码保持清晰可读。

### 2.5 单例模式 (Singleton Pattern)
*   **位置**: `FlightDataBus`, `UIStateBus`, `AlwaysOnTopCoordinator`
...
*   **载荷**: `FlightDataEvent` (包含 `EventPayload` 和 `HUDData`)
*   **触发**: 每帧 (Service Thread Loop, 默认 50ms)
*   **SLA**: 必须在 16ms 内处理完毕，禁止由 Event 触发重 IO 操作或复杂逻辑。
...
sequenceDiagram
    ...
    Note right of Svc: 20Hz (默认50ms，可配置)
    Game->>Svc: JSON Data
    Svc->>Svc: Calculate Physics + HUDData
    ...
    UI-->>UI: updateComponents(event.getHudData())
```
...
1.  **噪音**: UI 组件会被每秒 20 次的数据淹没，难以过滤出 "按钮点击" 事件。
...
3.  **完全重写 (Type C)**: `MiniHUDOverlay` 逻辑太复杂，无法复用通用逻辑。
...
## 5. 双数据访问模式 (Dual Data Access Patterns)

VoidMei 提供两种数据访问方式，针对不同场景优化。

### 5.1 TelemetrySource 接口 (高频数值)
...
// 使用
double ias = telemetrySource.getIAS();     // 零 GC
double gLoad = telemetrySource.getNy();    // 直接访问 G 载荷
...
// ❌ 错误：warnGLimit 字段不存在于 EventPayload
EventPayload payload = EventPayload.builder()
    .warnGLimit(sState.Ny > 5.0)  // warnGLimit 不存在！
    .build();

// ✅ 正确：使用 TelemetrySource.getNy() 获取 G 载荷
double gLoad = telemetrySource.getNy();
boolean isGWarning = gLoad > 5.0;
```

### 5.2 EventPayload (低频逻辑标志)

**获取方式**: 从 `FlightDataEvent` 获取
...
public void onFlightData(FlightDataEvent event) {
    ...
    boolean engineCheckDone = payload.engineCheckDone;
    boolean radioAltValid = payload.radioAltValid;
    boolean isDowningFlap = payload.isDowningFlap;
    String mapGrid = payload.mapGrid;
    String timeStr = payload.timeStr;
    int optStage = payload.optimalCompressorStage;      // 最优增压器阶段
...
@Override
public void onFlightData(FlightDataEvent event) {
    HUDData data = event.getHudData();  // 在 Service 线程预计算
    if (data == null) return;
...
## 6. Java 模式新手指南 (Java Patterns for Novices)

本节专为有 Java 基础但缺乏在复杂框架中实战经验的开发者准备。阅读本项目源码时，你会频繁通过以下模式与代码交互。

### 6.1 泛型与 Supplier 接口 (Generics & Suppliers)
...
### 6.2 Lambda 与 Functional Interfaces
你会看到大量的 `() -> action` 写法。
...
## 7. 端到端实战教学 (E2E Implementation Guide)

假设产品需求：新增一个 **"过载警告 (G-Force Warning)"** 功能，当过载超过 5G 时，在屏幕上显示红色警告。

### Step 1: 数据感知 (Service Layer)

**方式 A (推荐)**: 使用现有的 `TelemetrySource.getNy()` 读取 G 载荷

G 载荷数据已经通过 `TelemetrySource.getNy()` 暴露，无需修改 Service 层。

**方式 B**: 如需添加新的布尔标志到 EventPayload

```java
// 1. 在 EventPayload.java 中添加字段
public final boolean gForceWarning;

// 2. 在 Builder 中添加 setter
public Builder gForceWarning(boolean v) { this.gForceWarning = v; return this; }

// 3. 在 Service.updateGlobalPool() 中计算并设置
// 注意：使用已有的计算值，不要重复计算
EventPayload payload = EventPayload.builder()
    .gForceWarning(telemetrySource.getNy() > 5.0)
    // ... 其他字段 ...
    .build();
```
...
(panel "过载警告"
  ...
  (item "当前G值" :type data :target "getNy" :format "%s G")
)
```
...
让我们选择 **Type C (MiniHUD)** 的扩展方式，因为我们需要在屏幕中心画一个红色大字。
...
在 `MiniHUDOverlay.java` 的 `initComponentsLayout` 中：
```java
...
## 8. 专家级内幕与边缘情况 (Expert Internals & Edge Cases)

### 8.1 DSL 绑定的"黑盒" (DSL Binding Internals)
**流程图**:
```mermaid
graph TD
...
### 8.2 布局引擎 (Layout Engine)
**Type C** 委托给 `ModernHUDLayoutEngine` 处理堆叠。
*   **时序图**:
```mermaid
sequenceDiagram
    participant HUD as MiniHUDOverlay
    participant LE as ModernHUDLayoutEngine
...
*   **HUDData (推荐)**: 预计算的显示数据，在 Service 线程计算完成，避免 EDT 阻塞。
*   **State (慎用)**: 如果你必须进行极高性能的物理计算（如能量机动理论验证），你可以访问 `getState()` 获取原始对象。**警告**：该对象非线程安全，严禁修改。

### 8.5 配置持久化陷阱 (Persistence Trap)
...
## 9. 常见错误参考 (Common Mistakes)

### 9.1 错误的 TelemetrySource 访问
...
### 9.2 不存在的 EventPayload 字段

```java
// ❌ 错误：warnGLimit 字段不存在
payload.warnGLimit  // 编译错误！

// ✅ 正确：G 载荷通过 TelemetrySource 获取
double gLoad = telemetrySource.getNy();
boolean isGWarning = gLoad > 5.0;

// 或者从预计算的 HUDData 获取
HUDData data = event.getHudData();
boolean isGWarning = data.gLoad > 5.0;
```
...
*   **Q: 如何获取 G 载荷数据？**
    *   A: 使用 `telemetrySource.getNy()`，不要尝试访问 `payload.warnGLimit`（不存在）。
