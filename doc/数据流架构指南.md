...
**文件**: `src/prog/Service.java:1822-2082`
...
public static HUDData calculate(FlightDataEvent event,
    ...
    b.energyM = source.getEnergyJKg() / PhysicsConstants.g;
...
public class FlightDataEvent {
    ...
    @Deprecated  // 兼容旧代码，新代码请使用 getPayload()
    public Map<String, String> getData() { /* 从 payload 构建 Map */ }
}
```
...
public class MyOverlay extends BaseOverlay implements FlightDataListener {
    ...
    public void onFlightData(FlightDataEvent event) {
        ...
        SwingUtilities.invokeLater(() -> {
            // 从 EventPayload 获取类型安全的低频数据
            EventPayload payload = event.getPayload();
            String mapGrid = payload.mapGrid;        // 直接访问，无需解析
            boolean fatalWarn = payload.fatalWarn;    // 原生 boolean，无需 Boolean.parseBoolean
...
│  ├─ event.getPayload().fatalWarn → 类型安全逻辑标志          │
...
// 读取逻辑标志（低频，类型安全）
EventPayload payload = event.getPayload();
boolean isJet = payload.isJet;             // ✅ 原生 boolean，无需解析
String mapGrid = payload.mapGrid;          // ✅ 直接访问
boolean fatalWarn = payload.fatalWarn;     // ✅ 编译时检查
