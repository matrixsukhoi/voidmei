...
### ⚠️ 重要区分：性能基准 vs 准确性验证

| 测试类型 | 目的 | 参数来源 | 示例文件 |
|----------|------|----------|----------|
| **性能基准** (Benchmark) | 测量执行速度 | 简化的硬编码参数 | `BenchmarkPistonPower.java` |
| **准确性验证** (Validation) | 验证计算正确性 | 真实 FM 文件数据 | 本节模板 |

**常见错误**: 使用 Benchmark 文件的简化参数来验证计算准确性，导致误判功能正确性。
例如 `BenchmarkPistonPower.java` 使用 `critAlt=5200` 而真实 BF-109F-4 的临界高度是 ~2400m。

**正确做法**: 准确性验证必须：
1. 通过 `Blkx` 加载实际 FM 文件
2. 或使用 WAPC 的 `single_aircraft_calculator.py` 对比
