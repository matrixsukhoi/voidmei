...
fn index_out_of_range_returns_none() {
    // 基线: idx=2 / idx=3 超 parts.length → null
    assert_eq!(
...
fn negative_index_returns_none() {
    // 基线: idx=-1 → guard `index >= 0` 短路 → null
    assert_eq!(
...
fn non_numeric_item_returns_none() {
    // 基线: "[a, b]" idx=1 → null (无数字); "[x-4.5]" idx=1 → 越界 null
    assert_eq!(
...
fn no_brackets_returns_none() {
    // 基线: 无 '[' → LIST_PATTERN 不命中; 无 ']' 收尾 (未闭合) 同样不命中
    assert_eq!(
...
fn nested_bracket_takes_inner_prefix_as_content() {
    // 基线: "[[a],[b]]" idx=0 → 首个匹配内容为 "[a" (贪婪到首个 ']'), 无数字 → null
    assert_eq!(
...
fn java_split_trailing_empty_semantics() {
    // 基线: "[1,2,]" → 尾部空段被 Java split 移除, idx=1 = "2" → 2.0;
