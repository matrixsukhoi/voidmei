...
//! MultiListIndexRule / LambdaRule), 按 映射为 dyn trait。
...
pub trait ComparisonRule {
    ...
    ///
    /// - `raw_value`: the raw value string (e.g., "4644.0", "[144, 1167]")
    /// 返回: the extracted Double value, or null if extraction fails
    fn extract_value(&self, raw_value: Option<&str>) -> Option<f64>;

    /// 返回: true if lower values are better, false if higher values are better
