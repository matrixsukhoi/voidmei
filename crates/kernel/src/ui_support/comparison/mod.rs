//! ui.window.comparison.logic 包 (A 类, ): FM 属性对比的
//! 规则提取 (ComparisonRule/ComparisonRules + rules/*) 与胜负计算
//! (ComparisonCalculator)。
//! 窗口类 (CompactComparisonWindow 等) 属 C 类/后续批次, 不在本模块。
//! 同包 model/ComparisonData.java **亦为 A 类** (, 36 行, 纯数据
//! 结构), 只是不在本批 logic/ 翻译范围 —— 待 A 类放量批次补译, 勿漏。
//!
//! Java 包名 ui.window.comparison.logic → crate::ui_support::comparison (PORTING.md
