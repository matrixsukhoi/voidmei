...
//!
//! 重构波13 收敛点: ui/renderers/color.rs 与 config/configuration_service
//! 的两份同族实现归一至此。以 ui 侧实现为准 (历史基线 测试锚定,
//! 2026-08-26 对拍用例现锚于 ui renderers/color tests 与 configuration_service
//! tests 的 color_parse_matrix); 两份实现比对语义等价, 差异仅在 Option 形态
//! (kernel 旧版直接回落默认) 与切片写法, 无行为分歧。
//!
//! 边缘语义:
