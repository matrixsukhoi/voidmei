...
//! PORT(移仓备案): 原居 voidmei app_shell.rs; WYSIWYG reinit 链接通后
//! overlay 的 spec 工厂 reinit 闭包需按快照重建 MiniHUD (reinit_config 泛型
//! S: HUDSettings 的实参), 快照随 trait 同居本仓 (voidmei 经 kernel:: 引用)。
...
impl HudSettingsSnapshot {
    ...
    pub fn build<S: HUDSettings>(s: &S) -> Self {
        HudSettingsSnapshot {
            ...
            // 不取 get_font_name — 其 defaultFont 回退分支是 kernel 保真
