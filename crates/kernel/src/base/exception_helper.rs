...
//!
//! §2.13 — Java Thread.interrupt()/InterruptedException 在 Rust 无直接
//! 对应, 统一映射为 AtomicBool 停机标志:
//! - "恢复线程中断状态" ≡ 提前返回后**不清除**标志 (上游可观察到停机请求)。
//!
//! 停机标志生命周期契约 (跨文件, 按 只标注上报):
...
/// Java Thread.sleep 由 OS 直接唤醒, 无轮询; 此值为 §2.13 标志位的
...
/// 如果被中断，恢复中断状态但不抛出异常
///
/// §2.13 裁决 — "被中断"映射为停机标志置位: 分片睡眠并轮询标志,
...
pub fn sleep_quietly(stop: &AtomicBool, millis: u64) {
    ...
    while !stop.load(Ordering::SeqCst) {
        ...
        // 分片: 剩余时间与轮询片取小, 保证不睡过 deadline
        let chunk = std::cmp::min(deadline - now, POLL_INTERVAL);
        thread::sleep(chunk);
    }
    // 标志置位 → 静默提前返回; 标志保持置位 = 恢复中断状态
}

/// 运行极性辅助: 睡眠至 deadline 或运行标志翻 false (提前返回)。
///
/// [`sleep_quietly`] 的标志是 **true=停** 语义; VoiceWarning.doit 这类
