...
//!   RAII Drop 即注销 (订阅 guard 模式, 根治 记录的
...
///
/// Java `private static final FlightDataBus INSTANCE = new FlightDataBus();`
/// 单例字段不移植 —— 实例归调用方持有 (见模块头 PORT 说明)。
/// Java `private final List<FlightDataListener> listeners =
...
impl FlightDataBus {
    ...
    ///
    /// Java 按对象引用 `contains` 去重防重复注册 —— Rust 无引用等值,
    /// 每次调用产生独立订阅; 防重复注册由调用方只持有一个 Subscription 表达
    /// (对齐 FieldOverlay `subscribed` 标志的既有防御模式)。
    /// Java `publish` 内 per-listener try-catch 移植为
    ...
    ///
    /// PORT (强引用环警示, ): 本方法与"订阅方自持 Subscription"
    ...
    /// 返回送达数为 Rust 底座新增的诊断信息, Java 返回 void。
    ///
