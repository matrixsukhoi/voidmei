...
//!   → 构造参数注入。
//! - `prog.Service` (data, C 类波次; kernel 禁依赖 data — D3/D6 分层) →
...
//! (Service 线程每轮轮询原地更新); Rust 快照架构 (data ServiceData) 下由
...
///
/// Java 直接读 `xS` 的 public 字段/方法 (Service 为 C 类波次, kernel 不可依赖
/// data — D3/D6 分层), 本 trait = 该访问面的逐项签名化; data Service 波次
...
/// 组装契约 (data Controller 波次注意): run 需把整个结构 **move 进语音线程**,
