//! kernel: VoidMei 纯逻辑层, 11 域分组 (波8 定稿, 根留清零; 波9 shim 退役,
//! 全库统一 `crate::<域>::<模块>` / `kernel::<域>::<模块>` 唯一路径):
//! base(总线家族/事件/日志/工具/物理/大气) / config(配置栈) / game_api(8111 客户端+解析)
...
pub mod derived; // HUD 派生量/飞行分析/日志
pub mod fm; // FM 管理栈 + FM 数据 (data) + 功率模型族
pub mod formula; // 公式系统 (L0 registry/L1 编译/L2 规则引擎/manager)
pub mod game_api; // 8111 游戏本地 API: ureq 客户端 + serde 解析器 (波20 telemetry 更名)
