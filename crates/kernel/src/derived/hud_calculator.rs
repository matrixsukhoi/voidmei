...
//!    (overlay font.rs 的 `FontHandle::measure`, 语义对齐
...
fn apply_fm_warnings<S: HUDSettings>(
    ...
    if let Some(fmdata) = valid_fmdata {
        ...
        // 形态调用本方法 = Java 对位 NPE, panic 由调用点 (voidmei feed 的整帧
...
///
/// `java.awt.Font` 参数化为泛型 `F` (字体句柄由调用方定义, 如 overlay
