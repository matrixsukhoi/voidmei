...
//!
//! 依赖注入面 (Service/Controller 落 data, 本 crate 不可见):
//! - Service → 每次调用注入快照 [`FlightLogSnapshot`] (Service 轮询线程实时读字段的
//!   该时刻值); data 侧的快照构造面 = `data::service_loop::flight_log_snapshot`;
...
/// logTick 时刻 Service 字段的快照 (Service 落 data, 本 crate 以值注入)。
