...
fn vwing_fmdata() -> FmData {
    ...
    vw.full_flaps_wing = Some(FmParts {
        aoa_crit_high: 22.0,
        ..Default::default()
    });
...
fn descending_fmdata() -> FmData {
    ...
    dw.no_flaps_wing = Some(FmParts {
        aoa_crit_high: 16.0,
        ..Default::default()
    });
    dw.full_flaps_wing = Some(FmParts {
        aoa_crit_high: 20.0,
        ..Default::default()
    });
...
fn s1_no_state_returns_builder_defaults() {
    let src = mk_src(
        412.5, 0.62, 5300.0, 245.7, -13.2, 164.09, 14913.0, 0.83, 0.72, 0.66, 0.9, 144.0, true,
        false, 0.0,
    );
...
fn s5_spitfire_real_fm_values() {
    ...
    // 公式槽 基线 值 (数学由 data 位级对拍锚定, 此处锚定 HUD 接线)
...
fn s6_mach_mode_and_stall_warning() {
    ...
    let mut src = mk_src(
        300.0, 0.82, 800.0, 120.0, -999.9, 359.9, 5200.0, 0.55, 0.3, 0.2, 0.44, 155.0, true, false,
        0.0,
    );
...
fn s7_vwing_flap_allow_angle_branches() {
    ...
    for &(ias, want_angle, want_vne) in cases {
        ...
        assert_eq!(
            h.speed_bar_stall_ratio.to_bits(),
            0x3fc6_b8ce_0307_92ef,
            "ias={ias}"
        );
...
fn s9_descending_rows_early_return_and_clamp() {
    ...
    let payload = EventPayload::builder()
        .map_grid("B2".to_string())
        .time_str("01:02".to_string())
        .build();
    let mk = |ias: f64, flap: f64, vne: bool, man_bits: u64| {
        let mut s = mk_src(
            ias, 0.6, 2500.0, 700.0, -2.5, 270.0, 4100.0, 0.62, 0.4, 0.3, 0.5, 130.5, false, false,
            0.0,
        );
        s.flap_angle = flap;
    ...
    let h = calculate(
        ...
        Some(&ind),
        &payload,
        Some(&mk(640.0, 0.0, true, 0x3fc6_0a2c_1458_28b0)),
        Some(&dw),
        &hud,
        &COLORS,
    );
    ...
    let h = calculate(
        ...
        Some(&ind),
        &payload,
        Some(&mk(300.0, 50.0, true, 0x3fc6_0a2c_1458_28b0)),
        Some(&dw),
        &hud,
        &COLORS,
    );
    ...
    let h = calculate(
        ...
        Some(&ind),
        &payload,
        Some(&mk(150.0, 125.0, true, 0x3fc6_0a2c_1458_28b0)),
        Some(&dw),
        &hud,
        &COLORS,
    );
...
fn s10_invalid_fm_short_circuits() {
    ...
    bad.valid = false;
    let src = mk_src(
        250.0, 0.5, 1000.0, 300.0, 1.0, 90.0, 2000.0, 0.0, 0.0, 0.0, 0.0, 110.0, true, false, 0.0,
    );
...
fn java8_oracle_format_battery() {
    ...
    // %4s 空串
    assert_eq!(
        format!("{}{}{}", pad_width(String::new(), 4, false), "BRK", "GEA"),
        "    BRKGEA"
    );
    assert_eq!(
        format!("{}{}{}", pad_width(String::new(), 4, false), "", ""),
        "    "
    );
}
...
fn get_string_width_guards_and_delegation() {
    ...
    assert_eq!(
        get_string_width(None, Some(&FakeFont), |_, _| panic!("不应度量")),
        0
    );
    // text 空 → 0
    assert_eq!(
        get_string_width(Some(""), Some(&FakeFont), |_, _| panic!("不应度量")),
        0
    );
    // font null → 0
    let none_font: Option<&FakeFont> = None;
    assert_eq!(
        get_string_width(Some("ABC"), none_font, |_, _| panic!("不应度量")),
        0
    );
    // 正常: 委托调用方度量 (font + 原文直传)
    assert_eq!(
        get_string_width(Some("ABC"), Some(&FakeFont), |_, t| t.len() as i32 * 7),
        21
    );
}
