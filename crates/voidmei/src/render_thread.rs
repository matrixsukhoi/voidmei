...
//! 在 overlay::platform (窗口/托盘/热键) 与本 crate 的 winmm_player。
...
use kernel::audio::voice_resource_manager::VoiceResourceManager;
...
use kernel::base::java_compat::{current_time_millis, java_parse_boolean};
use kernel::base::logger;
use kernel::config::config_api::HudSettingsSnapshot;
use kernel::derived::hud_calculator::HudColors;
use kernel::fm::{FMHandle, FMManager};
use kernel::formula::registry::FormulaView; // var_value 取数唯一接口 (W10 后 TelemetrySource 已删)
use kernel::lang::Lang;

use overlay::overlays::minihud::{minihud_overlay_spec, MiniHudHandle};
use overlay::platform::host::{OverlayHost, OverlaySpec};
use overlay::widgets::{page_overlay_spec, PageHandle, PageSpecParams};
use overlay::platform::hotkey::HotkeyEvent;

#[cfg(target_os = "windows")]
use overlay::platform::tray::{TrayConfig, TrayHandler, TrayIcon};
...
fn strategy_of(doc: &kernel::config::json_model::PageDoc) -> ActivationStrategy {
    use kernel::config::json_model::ActivationReq;
...
impl kernel::platform::focus_monitor::AlwaysOnTopCoordinatorApi for ChannelFocusBridge {
...
impl overlay::platform::host::PositionStore for ChannelPositionStore {
...
fn interest_keys_of(doc: &kernel::config::json_model::PageDoc) -> Vec<String> {
    let mut keys: Vec<String> = doc.interest_keys.clone();
    for comp in &doc.components {
        if let Some(meta) = overlay::widgets::lookup_widget(&comp.r#type) {
...
fn interest_keys_of_opt(doc: Option<&kernel::config::json_model::PageDoc>) -> Vec<String> {
...
fn is_page_overlay_entry(doc: &kernel::config::json_model::PageDoc) -> bool {
...
fn register_page(
    ...
    env: &crate::env::Env,
    lang: &Rc<Lang>,
    params: &Rc<RefCell<overlay::platform::reinit::ReinitParams>>,
    shared: &ControllerShared,
    doc: &kernel::config::json_model::PageDoc,
) {
    strategies.insert(doc.id.clone(), strategy_of(doc));
    ...
    if let Some(kernel::config::json_model::DockSpec::BottomLeft { from_bottom }) = doc.dock {
...
fn assemble_page_spec(
    ...
    let gauge =
        overlay::widgets::GaugeCfg::from_params(&p, dpi, env.dpi.get_logical_screen_height());
    let font_size = overlay::widgets::page_font_size(24, doc.font.size_add, dpi);
    let hud = p.hud.clone();
    drop(p);
    ...
    let refresh: Box<dyn Fn() -> PageSpecParams> = Box::new(move || {
        ...
        let fs = overlay::widgets::page_font_size(24, doc.font.size_add, refresh_env_dpi);
        let hud = p.hud.clone();
        drop(p);
        PageSpecParams {
            // logical_height 沿用历史硬编码 1080 (P3 备案: 与初装配
            // get_logical_screen_height 的已知不一致)
            gauge_cfg: overlay::widgets::GaugeCfg::from_params(
...
fn refresh_font_path(_doc: &kernel::config::json_model::PageDoc) -> std::path::PathBuf {
...
pub(crate) struct OverlayRegSetup<'a> {
    ...
    /// 各 spec 工厂 reinit 闭包持引用读取 — 见 overlay reinit.rs 头注)
    pub(crate) params: &'a Rc<RefCell<overlay::platform::reinit::ReinitParams>>,
...
pub(crate) fn feed_overlays_live(
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        if let Some(h) = handles.minihud.as_ref() {
            ...
            let gc = overlay::render::palette::colors();
        ...
        let empty_data = kernel::derived::hud_data::HUDData::empty();
...
pub fn render_thread_main(cfg: RenderThreadConfig) {
    ...
    loop {
        ...
        if last_render.elapsed() >= Duration::from_millis(50) {
            ...
            let mut fm_changed: Option<Arc<kernel::fm::data::FmData>> = None;
            ...
            let sidecar_pages: Vec<(String, bool)> = {
                ...
                session
                    ...
                    .filter(|(id, _)| {
                        docs.iter().any(|d| {
                            &d.id == id
                                && d.dataface == kernel::config::json_model::DatafaceSpec::Sidecar
                        })
                    })
                    .map(|(id, _)| {
                        (
                ...
                || sidecar_pages.iter().any(|(id, _)| session.host.is_active(id))
            {
                ...
                for (page_id, manages_visibility) in &sidecar_pages {
                    ...
                    let mut sctx = overlay::widgets::SidecarCtx {
                        now_ms,
                        page_id,
                        fm: &session.fm,
                    ...
                    // 原子组件恒 None)
                    let mut action = overlay::widgets::SidecarAction::None;
                    {
                        ...
                        for cell in page_ref.cells.values() {
                            ...
                            if a != overlay::widgets::SidecarAction::None {
                                action = a;
                            }
                        }
                    }
                    ...
                    match action {
                        overlay::widgets::SidecarAction::None => {}
                        overlay::widgets::SidecarAction::Resize(w, h) => {
                            let _ = session.host.resize_entry(entry, w, h);
                        }
                        overlay::widgets::SidecarAction::SetVisible(v) => {
                            session.host.set_entry_visible(entry, v);
                        }
                        overlay::widgets::SidecarAction::SetVisibleResize(v, w, h) => {
                            session.host.set_entry_visible(entry, v);
                            let _ = session.host.resize_entry(entry, w, h);
                        }
                        overlay::widgets::SidecarAction::Close => {
        ...
        while let Ok(cmd) = session.ui_cmd_rx.try_recv() {
            match cmd {
                ...
                UiCommand::SetGlobalColors(c) => overlay::render::palette::set(c),
                UiCommand::SetAa(on) => overlay::render::palette::set_aa(on),
...
struct RenderSession {
    ...
    fm_data_rx: Receiver<Option<kernel::fm::data::FmData>>,
...
impl RenderSession {
    ...
    fn new(cfg: RenderThreadConfig) -> Self {
        ...
        // 颜色不一致根源)。须先于任何组件渲染
        overlay::render::palette::set(inputs.colors);
        overlay::render::palette::set_aa(inputs.aa);
        ...
        let params = Rc::new(RefCell::new(
            overlay::platform::reinit::ReinitParams::from(&inputs),
        ));
        ...
        let (fm_data_tx, fm_data_rx) =
            std::sync::mpsc::channel::<Option<kernel::fm::data::FmData>>();
    ...
    fn on_begin_edit_session(&mut self) {
        ...
        use overlay::platform::host::{EditBridge, EditMouse};
    ...
    fn on_reinit_overlays(&mut self, new_params: Box<overlay::platform::reinit::ReinitParams>) {
