...
use kernel::config::configuration_service::GlobalColors;
...
pub enum UiCommand {
    ...
    /// (MainForm 侧 ui W2 接线调 `AppShell::dispatch`)。
    StartGame,
    ...
    /// CONFIG_CHANGED 时即时读配置重建 [`overlay::platform::reinit::ReinitParams`]
    ...
    ReinitOverlays {
        params: Box<overlay::platform::reinit::ReinitParams>,
    },
...
pub enum MainEvent {
    ...
    /// 渲染线程已在发送前退出编辑形态
    EditCommitted {
        pages: Vec<kernel::config::json_model::PageDoc>,
        deleted: Vec<String>,
    },
