//! voidmei 组装层 (P5 批十四 W1): AppShell + Controller 生命周期核心 + 渲染线程入口。
...
//!   webui ShellForm (Tauri web 设置壳, D9) 的事件泵迭代 — main.rs 单循环驱动。
//! - 渲染线程: OverlayHost 全部 overlay 窗口 + 托盘消息窗口 + 热键事件消费
//!   (单泵共享; 热键钩子线程豁免记录见 `render_thread::render_thread_main` 头注)。
//! - Service 线程: data service_loop (8111 轮询, Controller 波次仅负责启停)。
...
// Controller 生命周期枚举 (波8 自 kernel 迁入: 唯一消费方是本 crate 的
...
// 表单 IPC 粘合层 (波11 自 main.rs 收编入 lib — bin 只剩纯入口; ui/webui
...
pub struct AppShell {
    ...
    /// 构造时经参数接线同源 (webui 直算命令经 tauri State 读)
...
impl AppShell {
    ...
    pub fn with_parts(parts: ShellParts) -> AppShell {
        ...
        AppShell {
            ...
            controller: None,
            render: None,
            formula_shared: webui::ipc::FormulaShared::default(),
        ...
        match ev {
            ...
            MainEvent::EditDiscarded => {
                ...
                if let Some(c) = self.controller.as_ref() {
                    ...
                    let params = overlay::platform::reinit::ReinitParams::from(&inputs);
            ...
            MainEvent::ConfigChanged(key) => {
                ...
                let params = overlay::platform::reinit::ReinitParams::from(
