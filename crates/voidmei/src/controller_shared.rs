...
use kernel::config::configuration_service::ControllerIntervals;
...
pub struct ControllerShared {
    ...
    pub live: RwLock<Option<Arc<data::frame::FrameStore>>>,
    ...
    /// PORT(B1 补偿): data 不外泄原始串 (http_client 轮询线程独占),
    ...
    /// data 后续波次补 raw_strings_valid 外泄后回收本补偿。
