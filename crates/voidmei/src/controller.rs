...
use kernel::derived::flight_log::FlightLogSlot;
use kernel::fm::{FMManager, FMStatus};
use kernel::game_api::client::GameApiClient;
use kernel::lang::Lang; // 波20: ureq 化更名

use data::service_loop::{
...
use overlay::platform::hotkey::{HotkeyManager, VC_P};
...
impl kernel::derived::flight_log::ControllerLogSink for LogonSink {
...
pub struct ControllerDeps {
    ...
    /// Service 时把会话实例换入 — webui 公式编辑器直算命令经 tauri State 读,
    /// 不经主线程 dispatcher (对位原 commands_formula 全局桥的覆盖写点)
    pub formula_shared: webui::ipc::FormulaShared,
}
...
pub struct Controller {
    ...
    subs: Vec<Subscription<kernel::base::bus::ui_state_bus::UiStateEvent>>,
    fm_sub: Option<Subscription<kernel::fm::FMHandle>>,
    ...
    formula_shared: webui::ipc::FormulaShared,
}
...
impl Controller {
    ...
    pub fn new(deps: ControllerDeps, is_initial_launch: bool) -> Controller {
        ...
        c.subs.push(ui_bus.subscribe(
            ...
            move |ev: &kernel::base::bus::ui_state_bus::UiStateEvent| {
        ...
        c.subs.push(ui_bus.subscribe(
            ...
            move |_ev: &kernel::base::bus::ui_state_bus::UiStateEvent| {
        ...
        c.fm_sub = Some(fm.fm_changed_bus().subscribe(move |handle| {
            ...
            // webui bridge_fm_changed 直连同一总线 (main.rs 接线)
    ...
    pub fn start(&mut self, release_main_form: &mut dyn FnMut()) {
        ...
        // tick 在 Service 轮询线程, 失焦回调经通道桥送渲染线程执行 host hide/show
        {
            let auto_hide = self
                ...
                .unwrap_or(false);
            let mut fm = kernel::platform::focus_monitor::FocusMonitor::new(
                Arc::new(overlay::platform::focus::WindowsFocusDetector),
        ...
        // live 集, 此处换入共享 cell 供 webui 命令线程访问 (启动桥实例退役,
        // 编辑保存已落盘由会话装载承接)
    ...
    /// (MainForm 自身的 saveConfig/hide 归 ui 侧, W2 接线。)
    ...
    // data 侧调用点已由 AppShell::pump → drive_from_live 顶替)
    // ------------------------------------------------------------------
    ...
    fn open_flight_log(&mut self) {
        ...
        // FlightLog 本体的降级行为不动 (kernel tests.rs "records/ 缺失" 用例钉住)。
        // 创建失败 (只读介质等) 不拦截 — 后续 init 按原降级路径走。
        let _ = std::fs::create_dir_all("records");
        let mut log = kernel::derived::flight_log::FlightLog::new();
        log.init(
            ...
            Arc::new(|t: &str| logger::info("FlightLog", t))
                as kernel::derived::flight_log::NotifySink,
    ...
    fn close_flight_log(&mut self) {
        ...
        if let Err(payload) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| log.close()))
        {
            ...
            let msg = kernel::base::exception_helper::panic_message_box(payload);
            logger::error(
                "Controller",
    ...
    /// changeS3/S4toS1 (data 侧不再回调 Controller — 本方法轮询帧快照
    ...
    /// data 外泄 raw_strings_valid 后两分支可逐字保真。
    ///
