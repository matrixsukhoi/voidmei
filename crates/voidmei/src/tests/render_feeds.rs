...
fn register_live_overlays_nine_window_entries() {
    let mut host = OverlayHost::with_factory(Box::new(|_cfg| {
        Ok(Box::new(NullWin) as Box<dyn overlay::platform::OverlayWindow>)
    }));
...
) -> Vec<(String, overlay::widgets::PageHandle)> {
    use overlay::widgets::{page_overlay_spec, GaugeCfg, PageSpecParams};
...
fn feed_overlays_live_updates_all_handles() {
    ...
    let lang = Rc::new(Lang::init_lang());
    let inputs = test_overlay_inputs();
    let (h_mini, _) = overlay::overlays::minihud::minihud_overlay_spec(
        false,
        50,
        &inputs.hud,
        1.0,
        &fonts.join("sarasa-mono-sc-bold.ttf"),
        &Rc::new(RefCell::new(overlay::platform::reinit::ReinitParams {
    ...
    // throttle/airbrake 各组件断言覆盖)
    {
        ...
        d.formula_values = kernel::formula::FormulaResults { values: vec![1.0] };
    }
    ...
    *shared.live.write().unwrap() = Some(frame_store_of(&d));
    ...
    use overlay::widgets::axes_atom::RudderBarWidget;
    use overlay::widgets::data_field::DataFieldWidget;
    use overlay::widgets::engine_gauge::EngineGaugeWidget;
    use overlay::widgets::gauges_composite::{AttitudeWidget, AxesWidget};
    use overlay::widgets::gear_flaps_atom::{FlapBarWidget, GearWarnWidget};
...
fn activation_keys_match_factory_default() {
    ...
    for page in &kernel::config::json_store::factory_default().pages {
...
fn factory_target_keys() -> Vec<String> {
    fn walk(rows: &[kernel::config::json_model::RowConfig], out: &mut Vec<String>) {
        for r in rows {
    ...
    let mut out = Vec::new();
    for p in &kernel::config::json_store::factory_default().panels {
        walk(&p.rows, &mut out);
    }
    out
}
...
fn focus_bridge_sends_commands_and_mirrors_hidden() {
    use kernel::platform::focus_monitor::AlwaysOnTopCoordinatorApi as _;
...
fn factory_pages_host_keys_unique() {
    let factory = kernel::config::json_store::factory_default();
...
fn reset_handles_preview_values_clears_live_residue() {
    ...
    use overlay::widgets::data_field::DataFieldWidget;
    use overlay::widgets::fm_list::FmListWidget;
    use overlay::widgets::gauges_composite::{AttitudeWidget, AxesWidget};
    ...
    // W-C: 派生量唯一真相 = 公式槽 (mach 经槽 0 注入 → 飞行信息行进 live 值)
    {
        ...
        d.formula_slots = std::sync::Arc::new(slots);
        d.formula_values = kernel::formula::FormulaResults { values: vec![0.72] };
    }
    let frame = data::frame::Frame::from_service_data(&d);
    let payload = EventPayload::builder().build();
...
fn fm_page_interest_keys_verbatim_java_controller() {
    let keys = {
        let cfg = kernel::config::json_store::factory_default();
        cfg.pages
            .iter()
