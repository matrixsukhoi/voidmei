...
impl kernel::audio::voice_resource_manager::SoundPlayer for CountingPlayer {
    fn open_clip(
        &self,
        path: &std::path::Path,
    ) -> Result<
...
impl kernel::audio::voice_resource_manager::SoundClip for CountingClip {
    fn start(&self) {
        *self
            .plays
            .lock()
...
fn refresh_previews_stop_voice_warn_session() {
    ...
    // 静默 None)
    fn probe_deliveries(bus: &kernel::base::bus::ui_state_bus::UIStateBus) -> usize {
        bus.publish(ui_state_events::CONFIG_CHANGED, Some("W1Probe"), None)
    }
    fn wait_deliveries(bus: &kernel::base::bus::ui_state_bus::UIStateBus, want: usize) -> bool {
