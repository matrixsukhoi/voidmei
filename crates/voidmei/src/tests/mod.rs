//! voidmei 集成测试 (波11 分片: 共享夹具/助手在本文件, 用例按主题分片 —
//! lifecycle / debounce_config / render_feeds / voice; 分片经 `use super::*`
//! 取共享面, 先例 overlay overlays/fields_tests)。
...
use kernel::base::bus::EventBus; // FmChangedBus 底座 (EventBus<FMHandle>) 的构造面
...
fn frame_store_of(d: &ServiceData) -> Arc<data::frame::FrameStore> {
...
fn update_live_frame(store: &data::frame::FrameStore, d: &ServiceData) {
    store.publish(data::frame::Frame::from_service_data(d));
}

/// tmp 配置文件 (kernel configuration_service 测试同款惯例)
...
/// 行/panel 构造速记
fn trow(target: &str, value: bool) -> kernel::config::json_model::RowConfig {
    kernel::config::json_model::RowConfig {
        label: target.to_string(),
        r#type: "SWITCH".to_string(),
        property: Some(target.to_string()),
        value: Some(kernel::config::json_model::ConfigValue::Bool(value)),
        default_value: Some(kernel::config::json_model::ConfigValue::Bool(value)),
        ..kernel::config::json_model::RowConfig::default()
    }
}

fn tpanel(
    title: &str,
    rows: Vec<kernel::config::json_model::RowConfig>,
) -> kernel::config::json_model::GroupConfig {
    kernel::config::json_model::GroupConfig {
        title: title.to_string(),
        visible: true,
        rows,
        ..kernel::config::json_model::GroupConfig::default()
    }
}

/// 数值行 (slider 语义)
fn tint(target: &str, value: i32) -> kernel::config::json_model::RowConfig {
    kernel::config::json_model::RowConfig {
        label: target.to_string(),
        r#type: "SLIDER".to_string(),
        property: Some(target.to_string()),
        value: Some(kernel::config::json_model::ConfigValue::Int(value)),
        default_value: Some(kernel::config::json_model::ConfigValue::Int(value)),
        ..kernel::config::json_model::RowConfig::default()
    }
}

/// 字符串行 (combo 语义)
fn tstr(target: &str, value: &str) -> kernel::config::json_model::RowConfig {
    kernel::config::json_model::RowConfig {
        label: target.to_string(),
        r#type: "COMBO".to_string(),
        property: Some(target.to_string()),
        value: Some(kernel::config::json_model::ConfigValue::Str(value.to_string())),
        default_value: Some(kernel::config::json_model::ConfigValue::Str(value.to_string())),
        ..kernel::config::json_model::RowConfig::default()
    }
}

/// fixture 树 (crosshairSwitch=true / enableEngineControl=false / autoStart=false)
fn test_panels() -> Vec<kernel::config::json_model::GroupConfig> {
    vec![tpanel(
        "T",
        vec![
...
fn auto_start_panels() -> Vec<kernel::config::json_model::GroupConfig> {
...
fn fixture_full(ms: u64, panels: Vec<kernel::config::json_model::GroupConfig>) -> AppShell {
    ...
    let mut shell = AppShell::with_parts(ShellParts {
        ...
        fm: Arc::new(FMManager::new(
            Arc::new(kernel::base::bus::EventBus::new()),
        )),
        hotkey,
        hotkey_rx,
...
fn publish_ui_event(
    bus: &kernel::base::bus::ui_state_bus::UIStateBus,
    event_type: &str,
    data: &str,
) {
...
impl overlay::platform::OverlayWindow for NullWin {
    ...
    fn poll_event(&mut self) -> Option<overlay::platform::OverlayEvent> {
        None
    }
...
fn test_overlay_inputs() -> OverlayInputs {
    OverlayInputs {
        ...
        pages: kernel::config::json_store::factory_pages_arc(),
...
/// "告警键触发 play 路径" 的观测探针 (kernel voice_warning tests 的
