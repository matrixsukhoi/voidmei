...
use kernel::config::config_api::{ConfigProvider, HudSettingsSnapshot, OverlaySettings};
...
/// 重建经 [`overlay::platform::reinit::ReinitParams`] 走 `UiCommand::ReinitOverlays`
/// (见 overlay reinit.rs 头注) — 主线程 CONFIG_CHANGED 时即时重建参数包直送
...
pub struct OverlayInputs {
    ...
    pub pages: std::sync::Arc<Vec<kernel::config::json_model::PageDoc>>,
...
fn all_bool_rows(config: &ConfigurationService) -> HashMap<String, bool> {
    ...
    fn walk(rows: &[kernel::config::json_model::RowConfig], out: &mut HashMap<String, bool>) {
        for r in rows {
            if let (Some(key), Some(kernel::config::json_model::ConfigValue::Bool(v))) =
                (&r.property, &r.value)
            {
    ...
    for title in kernel::config::json_store::factory()
        .panels
        .iter()
        .map(|p| p.title.clone())
    {
...
impl From<&OverlayInputs> for overlay::platform::reinit::ReinitParams {
    fn from(i: &OverlayInputs) -> Self {
        use overlay::platform::reinit::{AttitudeGroup, EdgeGroup};
        overlay::platform::reinit::ReinitParams {
