...
use kernel::audio::voice_warning::{VoiceWarning, VoiceWarningService};
...
fn refresh_voice_config_snapshot(
    ...
    for ty in kernel::audio::voice_alert_type::ALL {
        ...
        let cfg_key = kernel::audio::VoicePackConfig::with_voice_prefix(Some(ty.get_key()))
            .expect("告警键非 null");
        m.insert(
            cfg_key.clone(),
...
struct LiveVoiceService {
    frames: Arc<data::frame::FrameStore>,
}
...
impl VoiceWarningService for LiveVoiceService {
    ...
    fn is_eng_jet(&self) -> bool {
        ...
        self.frames
            .latest()
            .is_some_and(|f| f.engine.engine_type == data::service_fields::EngineType::Jet)
    }
    ...
    fn s_state(&self) -> kernel::game_api::parser::State {
    ...
    fn s_indic(&self) -> kernel::game_api::parser::Indicators {
        self.frames
            .latest()
...
pub(crate) fn voice_warn_refresh_reaches(changed_key: Option<&str>) -> bool {
    overlay::platform::host::is_global_config(changed_key)
...
pub(crate) fn open_voice_warning(
    ...
    live: Option<Arc<data::frame::FrameStore>>,
