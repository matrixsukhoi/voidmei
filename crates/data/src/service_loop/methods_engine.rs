...
mod tests {
    ...
    use kernel::fm::piston_model::CompressorStageParams;
    use kernel::fm::FMManager;
    use kernel::formula::registry::FormulaView as _; // var_value 取数
    ...
    fn flap_takeover_state_machine() {
        ...
        let defs = kernel::formula::persistence::load_merged(
    ...
    fn flap_allow_speed_angle_oracle() {
        ...
        // 60%: 档间插值 → 284.0
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(60, true, fm.fmdata.as_ref()),
            284.0
        );
        // 50%: 相等档位 (50 == 0.5*100) → 直接返回首档速度 290.0
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(50, false, fm.fmdata.as_ref()),
            290.0
        );
        // 30% (i=-1): 下襟翼越级 → 首档速度 290.0; 非下襟翼 → Double.MAX_VALUE
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(30, true, fm.fmdata.as_ref()),
            290.0
        );
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(30, false, fm.fmdata.as_ref()),
            f64::MAX
        );
        // 120%: 超表外插 (Java 无 clamp): 290 + 70*(-0.6) = 248.0
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(120, false, fm.fmdata.as_ref()),
            248.0
        );
        // flapPercent=0 早退 (先于 blkx 判定)
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(0, true, fm.fmdata.as_ref()),
            f64::MAX
        );
        ...
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(270.0, false, fm.fmdata.as_ref()),
            83.33333333333334
        );
        // 290: 相等 → 首档角 0.5*100 = 50.0
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(290.0, false, fm.fmdata.as_ref()),
            50.0
        );
        // 350 (i=0 分支): 50 + 60*(-5/3) = -50 → normFlapAngle → 0
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(350.0, false, fm.fmdata.as_ref()),
            0.0
        );
        // 100 (低速外插): 50 + (100-290)*(-5/3) = 366.6.. → 封顶 125
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(100.0, false, fm.fmdata.as_ref()),
            125.0
        );
        // ias=0 早退
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(0.0, true, fm.fmdata.as_ref()),
            125.0
        );
        ...
        assert_eq!(
            kernel::fm::data::get_flap_allow_speed(50, false, unr.fmdata.as_ref()),
            f64::MAX
        );
        assert_eq!(
            kernel::fm::data::get_flap_allow_angle(300.0, false, unr.fmdata.as_ref()),
            125.0
        );
    }
    ...
    fn real_fmloader_spitfire_pipeline() {
        ...
        kernel::fm::data_paths::set_data_root(&root);
        let fm = kernel::fm::loader::load(Some("spitfire_f24"));
        ...
        let got = kernel::fm::data::get_flap_allow_speed(flap_pct, false, Some(fmdata));
