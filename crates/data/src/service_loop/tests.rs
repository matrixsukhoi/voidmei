...
fn update_speed_ratio_and_stall_speed_oracle() {
    ...
    let fmdata = kernel::fm::data::FmData::parse_named_json(&phys, "fm/spitfire_f24.blk").unwrap();
...
fn mock_e2e_s2_preview_live() {
    ...
    assert!(n > 0, "3 秒内应收到 FlightDataEvent (实际 {n})");
    {
        ...
        // 波20 serde 化: vario f64 直读 (原 f32 拓宽位级), valid 真实 bool
        assert!(
            (d.n_vy - (-7.342558)).abs() < 1e-6,
...
/// ControllerLogSink no-op (本测试不覆盖 init 失败路径, 见 kernel flight_log 测试)
struct NopSink;
impl kernel::derived::flight_log::ControllerLogSink for NopSink {
...
fn flight_log_tick_writes_rows_and_close_flushes() {
    ...
    // 本测试二进制内并行线程共享进程 CWD → 串行化 + 用完恢复 (kernel
    // flight_log.rs 同款; 与 kernel 测试是不同进程, 天然互斥)
    ...
    let r = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let mut fl = kernel::derived::flight_log::FlightLog::new();
...
fn formula_step_evaluates_and_guards_mach() {
    ...
    let fm = kernel::fm::FMHandle::UNRESOLVED;
    svc.formula_step(&fm);
    {
        let d = svc.data.read().unwrap();
    ...
    let fmdata = {
        let mut b = kernel::fm::data::FmData::default();
        b.valid = true;
        b
    };
    let fm_ready = kernel::fm::FMHandle::ready(Some("mock".into()), Some(fmdata), 0.0, 0.0, None);
...
fn frame_replay_formula_matches_oracle() {
    ...
    let defs = vec![kernel::formula::FormulaDef {
...
/// W2 Deriver 消解的位级回归锚: 出厂公式集接管 an/sep/turn_rate/turn_rds/
/// acceleration 后, 20 帧字段值逐位锁定 (测试数据同 replay_state_json)。
/// 基准沿革: 2026-08-29 抓取 Deriver 输出 → 波20 serde 化 (f32 拓宽退役,
/// 数值 f64 直读) 后于 2026-09-03 重录, 第 8~10 位有效数字有预期漂移。
#[test]
...
fn panel_targets_via_short_names() {
    use kernel::formula::registry::FormulaView as _;
    let mut svc = new_service();
    for i in 0..20 {
        {
            let mut d = svc.data.write().unwrap();
