...
//! HTTP/parser 用 kernel game_api 域 (GameApiClient / parser::{State, Indicators})。
...
use kernel::base::calc_helper::SimpleMovingAverage;
...
use kernel::derived::flight_analyzer::AnalyzerService;
use kernel::derived::flight_log::{FlightLogSlot, FlightLogSnapshot};
use kernel::fm::{FMHandle, FMManager};
use kernel::formula::registry::FormulaView as _; // var_value 取数唯一接口
use kernel::game_api::client::GameApiClient; // 波20: ureq 化更名
use kernel::game_api::parser::{Indicators, MapInfo, MapObj, State};
...
use kernel::game_api::parser::F_INVALID; // 波20: 哨兵常量迁至 parser
...
pub(crate) fn session_inputs(d: &ServiceData) -> kernel::formula::registry::SessionInputs {
    kernel::formula::registry::SessionInputs {
        ...
        // 四判定 = EngineScalars 的 *_live 方法 (flag 守卫语义见其 doc;
        // PROP||TURBOPROP 的 is_prop 语义随方法走)
        is_jet: d.engine.is_jet_live(),
        is_prop: d.engine.is_prop_live(),
        is_piston: d.engine.is_piston_live(),
        is_turboprop: d.engine.is_turboprop_live(),
...
// Java 标准库/panic 语义助手已收敛 kernel (java_compat::current_time_millis /
...
impl Service {
    ...
    pub fn new(config: ServiceConfig, fm_manager: Arc<FMManager>, bus: Arc<FlightDataBus>) -> Self {
        ...
        let formula = Arc::new(kernel::formula::FormulaManager::new());
        ...
        let mut svc = Service {
            ...
            formula,
            rule_engine: kernel::formula::rules::RuleEngine::new(),
            last_fm_name: None,
        ...
        {
            let builtin =
                std::fs::read_to_string(kernel::formula::persistence::BUILTIN_FORMULAS_PATH)
                    .unwrap_or_default();
            let user = std::fs::read_to_string(kernel::formula::persistence::USER_FORMULAS_PATH)
                .unwrap_or_default();
            let mut rules = kernel::formula::persistence::parse_rules(&builtin);
            rules.extend(kernel::formula::persistence::parse_rules(&user));
            svc.rule_engine
                .install(&rules, kernel::formula::registry());
        }
    ...
    /// 见 struct 字段注)。
    pub fn set_focus_monitor(&mut self, fm: kernel::platform::focus_monitor::FocusMonitor) {
    ...
    fn poll_once(&mut self) -> Flow {
        ...
        if diff_time1 >= 10 * freq {
            ...
            if !port_occupied {
                ...
                self.http_client
                    .get_req_map_obj_result(request_dest_bkp(&self.config));
            }
    ...
    fn detect_player_live(&mut self) {
        ...
        // None 不可达, Option 域内比较 (kernel indicators.rs 既有防御)
    ...
    fn calculate(&mut self) {
        ...
        self.apply(|d| {
            ...
            d.altm.d_radio_alt = (d.ratio_1 * d.altm.d_radio_alt)
                + d.ratio * 1000.0 * (d.altm.radio_alt - d.altm.p_radio_alt)
                    / actual_interval_ms as f64;
    ...
    fn formula_step(&mut self, fm: &FMHandle) {
        ...
        let (results, slots, snap, interval_ms) = self.with_snapshot(|d| {
            let meta = kernel::formula::MetaInputs {
            ...
            let raw = kernel::formula::registry::RawInputs {
            ...
            let (results, snap) =
                self.formula
                    .eval_frame(&raw, &session, &meta, current_time_millis() as u64);
            (
                results,
        ...
        // L2 规则求值 (公式之后同快照; 触发事件写 ServiceData, 消费面 voidmei 接)
        let triggers =
            self.rule_engine
                .eval(&snap, &results, current_time_millis() as u64, interval_ms);
        self.apply(|d| {
            ...
            for f in &set.formulas {
                ...
                let Some(&slot) = set.slots.get(&f.def.name) else {
                    continue;
                };
            ...
            if fuel_delta > 0.0 {
                ...
                if !d.fuel.low_acc_fuel {
                    ...
                    if d.fuel.fueltime > 0 {
                        d.fuel.fueltime = if d.fuel.fueltime < tmpft {
                            d.fuel.fueltime
                        } else {
                            tmpft
                        };
        ...
        self.apply(|d| {
            ...
            if let Some(fmdata) = fm.fmdata.as_ref() {
                if fmdata.nitro != 0.0 && nitro_eng_nr != 0 {
                    d.engine.s_wep_time_val =
                        ((fmdata.nitro / fmdata.nitro_decr - (wep_time / 1000) as f64)
    ...
    fn update_engine_state(&mut self, fm: &FMHandle) {
        ...
        // 输入快照 + 引擎循环 (锁外算, §2.8)
        let (is_jet, total_hp, total_hp_eff, total_thrust, avgeff) = self.with_snapshot(|d| {
        ...
        self.apply(|d| {
            ...
            let peak_power = if fm.fmdata.is_some() {
                fm.peak_wep_power
            } else {
                0.0
            };
            let peak = if fm.fmdata.is_some() {
                fm.peak_thrust
            } else {
                0.0
            };
...
// ------------------------------------------------------------------
// FlightLog 接线面 (D6 边界的 data 侧落地, 见 flight_log.rs 模块头 PORT)
// ------------------------------------------------------------------
...
/// data 侧 impl)。getter 逐字段读锁 (合同义务 2: 逐字段调用对应 Java 逐字段
