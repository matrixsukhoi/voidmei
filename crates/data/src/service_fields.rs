...
use kernel::game_api::parser::{Indicators, MapInfo, State};

pub use kernel::base::engine_type::EngineType; // 波21: 迁 kernel (data re-export 保既有路径)
...
pub struct ServiceData {
    ...
    pub formula_values: kernel::formula::FormulaResults,
    /// 公式名→结果槽 (formula_step 与 values 同步写; overlay 绑定解析用)
    pub formula_slots: std::sync::Arc<std::collections::HashMap<String, u16>>,
    /// L2 规则本帧触发事件 (formula_step 产出; 消费面 voidmei toast/语音链)
    pub rule_triggers: Vec<kernel::formula::rules::RuleTriggered>,
}
...
impl kernel::formula::registry::FormulaView for ServiceData {
    fn var_value(&self, name: &str) -> Option<f64> {
        ...
        use kernel::formula::registry::{registry, VarSrc};
        ...
        let v = match src {
            ...
            VarSrc::Meta(m) => match m {
                kernel::formula::registry::MetaVar::IntervalMs => {
                    self.actual_interval_ms.max(1) as f64
                }
                kernel::formula::registry::MetaVar::Freq => self.freq as f64,
                kernel::formula::registry::MetaVar::FmLoaded => {
