...
fn var_value_state_passthrough() {
    ...
    d.s_state = Some(kernel::game_api::parser::State::default());
    d.s_state.as_mut().unwrap().ias = 474;
    d.s_indic = Some(kernel::game_api::parser::Indicators::default());
    use kernel::formula::registry::FormulaView as _;
...
fn var_value_formula_slot_and_single_name() {
    use kernel::formula::{FormulaDef, FormulaManager};
    ...
    let raw = kernel::formula::registry::RawInputs::default();
...
fn var_value_booster_wep_fuel_registry_vars() {
    let mut d = ServiceData::default();
    let mut s = kernel::game_api::parser::State::default();
    s.mfuel_1 = 300.0;
    s.mfuel0_1 = 400.0;
    d.s_state = Some(s);
    use kernel::formula::registry::FormulaView as _;
    ...
    let mut s2 = kernel::game_api::parser::State::default();
