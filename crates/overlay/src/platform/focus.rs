...
//! 实现 kernel `focus_monitor::FocusDetector` trait (Windows 腿落地)。
//! 跨平台分派腿 (FocusDetector.java 的 os.name 判定, A 类) 不在本文件,
//! 见 kernel focus_monitor.rs trait 注 (FocusDetector trait 归属维持在那侧)。
...
mod win {
    ...
    use kernel::platform::focus_monitor::FocusDetector;
