...
impl Default for TrayConfig {
    /// 默认标签取 kernel Lang 静态表 — Java: initSystemTray 运行时读 Lang 字段
    fn default() -> Self {
...
fn dispatch_activate() {
    ...
    if TRAY_CLICK_PROCESSING
        ...
        .is_err()
    {
        kernel::base::logger::info("Application", "Ignoring duplicate tray click");
        return;
    }
...
unsafe fn readd_icon() {
    ...
    if Shell_NotifyIconW(NIM_ADD, &nid).as_bool() {
        kernel::base::logger::info("Application", "任务栏重启, 托盘图标已重挂");
    }
}
...
unsafe fn load_icon_or_default(path: &Path) -> Result<(HICON, bool), String> {
    match load_icon(path) {
        Ok(h) => Ok((h, true)),
        Err(e) => {
            kernel::base::logger::warn_default(&format!("图标加载失败回退默认: {}", e));
        ...
        unsafe {
            ...
            *shared = Some(TrayShared {
            ...
            if !added {
                ...
                kernel::base::logger::warn("系统托盘", "托盘加入失败 (NIM_ADD), 程序继续运行");
                // Java `debugPrint(Lang.failaddtoTray)` → Logger.info("Legacy", ...)
                kernel::base::logger::info(
                    "Legacy",
                    kernel::lang::Lang::init_lang().failaddto_tray,
                );
            }
