...
use kernel::base::format::{fmt_f, java_string_format, FmtArg};
...
/// 求值语义: 谓词/比较/= 容差(0.0001 边界)/Not/And — 与 kernel 求值器一致
#[test]
fn vis_expr_semantics() {
    let t = MockTele::default(); // piston=true, jet=false
    assert!(!kernel::ui_support::row_def::Cond::IsJetEngine.eval(&t, 0.0));
    assert!(kernel::ui_support::row_def::Cond::IsPistonEngine.eval(&t, 0.0));
    assert!(!kernel::ui_support::row_def::Cond::HasWep.eval(&t, 0.0));
    assert!(kernel::ui_support::row_def::Cond::Gt(0.0).eval(&t, 0.1));
    assert!(!kernel::ui_support::row_def::Cond::Gt(0.0).eval(&t, 0.0));
    assert!(kernel::ui_support::row_def::Cond::Lte(0.0).eval(&t, 0.0));
    assert!(kernel::ui_support::row_def::Cond::Eq(1.0).eval(&t, 1.00001));
    assert!(!kernel::ui_support::row_def::Cond::Eq(1.0).eval(&t, 1.0002));
    // f64 边界: 字面量 1.0001-1.0 实际差 ≈ 9.9999e-5 < 0.0001 → 视为相等
    // (kernel 求值器测试同款 基线: "(= value 1)" 对 1.0001 为 true)
    assert!(kernel::ui_support::row_def::Cond::Eq(1.0).eval(&t, 1.0001));
    assert!(!kernel::ui_support::row_def::Cond::NotEq(1.0).eval(&t, 1.0001));
    assert!(!kernel::ui_support::row_def::Cond::NotEq(1.0).eval(&t, 1.0));
    let not_jet = kernel::ui_support::row_def::Cond::Not(Box::new(
        kernel::ui_support::row_def::Cond::IsJetEngine,
    ));
    assert!(not_jet.eval(&t, 0.0));
    let and = kernel::ui_support::row_def::Cond::And(
        Box::new(kernel::ui_support::row_def::Cond::IsPistonEngine),
        Box::new(kernel::ui_support::row_def::Cond::NotEq(1.0)),
    );
...
// ---- GearFlaps: state 级测试已随面板拆解退役 (flapbar/warn 原子组件 +
// ---- 数据推进断言见 voidmei render_feeds::feed_overlays_live_updates_all_handles) ----

// ---- live 喂数形态工厂测试已随旧 spec 工厂退役删除 (W3 组件化: 句柄共享/
// ---- reinit 语义由 widgets 域组件 + page_overlay_spec.refresh 面承载, 数据推进
// ---- 断言见 voidmei render_feeds::feed_overlays_live_updates_all_handles) ----
...
fn four_overlays_mount_into_overlay_host() {
    ...
    // widgets/fm_field; 挂载链断言见 voidmei render_feeds 的 fm-list 页用例)
...
// live 工厂共享态/reinit 测试已随旧 spec 工厂退役删除 (W3 组件化: gauges_
// composite 的 AxesWidget 包本 state, WYSIWYG 重建走 page_overlay_spec.refresh;
// 数据推进/reinit 链断言见 voidmei render_feeds)

/// CloseAllOverlays 数据面重置 (reset_preview 语义, 组件 reset_preview 的 state 面):
