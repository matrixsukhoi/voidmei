...
use kernel::base::format::{fmt_f, pad_width};
use kernel::base::event::event_payload::EventPayload;
use kernel::config::config_api::{HUDSettings, HudSettingsSnapshot};
use kernel::config::json_model::PageDoc;
use kernel::derived::hud_calculator::{self, HudColors};
...
impl MiniHudOverlay {
    ...
    pub fn init<S: HUDSettings>(
        ...
        kernel::base::logger::info("MinimalHUD", "init called");
    ...
    pub fn reinit_config<S: HUDSettings>(
        ...
        kernel::base::logger::info("MinimalHUD", "reinitConfig called");
...
pub fn minihud_overlay_spec<S: HUDSettings>(
    ...
    let reinit: ReinitFn = Box::new(move || {
        ...
        let Some(doc) = doc else {
            kernel::base::logger::warn("MinimalHUD", "reinit: 出厂页缺失, 保持旧状态");
            return None;
        };
        let mut o = reinit_handle.borrow_mut();
        if let Err(e) = o.reinit_config(&hud, &doc) {
            kernel::base::logger::error("MinimalHUD", &format!("reinit_config 失败: {}", e));
            return None;
        }
