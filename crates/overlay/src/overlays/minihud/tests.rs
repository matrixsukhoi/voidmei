...
use kernel::base::format::fmt_f;
use kernel::config::config_api::OverlaySettings;
use kernel::derived::hud_data::{Builder, HUDData};
...
fn doc() -> kernel::config::json_model::PageDoc {
    kernel::config::json_store::factory()
        .pages
        .iter()
...
impl FormulaView for MockSrc {
    ...
    fn var_value(&self, name: &str) -> Option<f64> {
        match name {
            ...
            _ => kernel::formula::registry::registry()
                .lookup(name)
                .map(|_| 0.0),
        }
    }
...
/// 组件)。喂数链 (voidmei feed_overlays_live) 曾以 None/None 重构事件, 该块整体
