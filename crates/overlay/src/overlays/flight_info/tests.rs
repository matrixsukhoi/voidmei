...
fn factory_page_targets_all_reachable() {
    let pages = &kernel::config::json_store::factory().pages;
    let mut n = 0;
    for page in pages {
        for c in &page.components {
            ...
            let (var, _) = kernel::formula::resolve_target(target)
                .unwrap_or((kernel::formula::TargetVar::Var(0), 1.0));
            let t = match &var {
                kernel::formula::TargetVar::Var(vid) => {
                    kernel::formula::registry::registry().vars[*vid as usize].name
                }
                kernel::formula::TargetVar::Formula(name) => name,
            };
            assert!(
