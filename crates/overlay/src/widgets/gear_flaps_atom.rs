...
use kernel::base::format::java_round_f64;
...
fn f_gear_warn(props: &serde_json::Value, fctx: &FactoryCtx) -> Result<Box<dyn HudWidget>, String> {
    let cfg: GaugeCfg = fctx.gauge_cfg.cloned().unwrap_or_default();
    let font_size = font_size_of(fctx, &cfg, props);
    let half = kernel::base::format::java_round_f32(font_size as f32 / 2.0);
...
impl GearWarnWidget {
    ...
    fn update_warn(&mut self, s: &dyn kernel::formula::registry::FormulaView, lang: &Lang) {
