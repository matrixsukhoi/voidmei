...
//! verdict 文案与失败计数泛型化为 [`Report`]。仅 ui bin (--headless) 与
//! CI 验收消费, 不进状态机核心。
...
pub fn run_headless(persist_path: Option<String>) -> i32 {
    ...
    let tmp = std::env::temp_dir()
        ...
        .to_string_lossy()
        .into_owned();
    config.install_for_test(kernel::config::json_store::factory_default().panels, &tmp);
    let mut state = MainFormState::new(config, Arc::clone(&bus));
    println!(
        "ui: --headless 表单构建成功: {} panels / {} rows (源: 出厂默认 ⊕ delta)",
        state.panel_count(),
        state.row_count()
    );
    ...
    for (row_type, drive) in STEPS {
        if let Some((panel, key)) = state.first_row_of_type(row_type) {
            ...
            println!("ui: [{}] {}", report.record(ok), text);
        } else {
            println!("ui: [SKIP] 出厂配置无 {row_type} 行");
        }
    }
    ...
    // 6) 保存链路: delta 落盘 + 广播 CONFIG_CHANGED("ui_layout.cfg")
    {
        ...
        println!(
            "ui: [{}] Save 广播 CONFIG_CHANGED(\"ui_layout.cfg\")",
    ...
    if let Some(p) = persist_path.as_deref() {
        ...
        println!("ui: --persist 基线已落盘至 {p}");
    }

    if report.failures == 0 {
        println!("ui: --headless 状态机全部通过");
        0
    } else {
        eprintln!("ui: --headless 失败 {} 项", report.failures);
        1
    }
}
