...
//!
//! 全部**直算模式** (commands_comparison 先例): 计算面只依赖 kernel 的
//! formula 模块 (FormulaManager 自身线程安全), 不经主线程 dispatcher,
//! voidmei 的 form_dispatch 零改动。
//!
//! 共享态 (E11 注入形态统一): [`FormulaShared`] — FormRuntime 字段, 经 tauri
//! State 分发到本层 (voidmei 装配方写: 启动桥注入 + 会话 start 覆盖 Service
...
pub async fn get_var_catalog(
    ...
    let mut out: Vec<VarCatalogEntryDto> = kernel::formula::registry()
        .catalog()
        .into_iter()
    ...
    // 公式产出变量 (State 恒有 manager — 出厂空集时循环体自然为空)
    {
        ...
        let set = mgr.current();
        let snap = mgr.last_snapshot();
        let reg = kernel::formula::registry();
