//! webui-selftest bin (D9 阶段① POC 验收):
...
fn main() {
    ...
    let mut form = match ShellForm::new(
        ...
        webui::ipc::FormulaShared::default(),
    ) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("webui-selftest: {e}");
            std::process::exit(2);
        }
    };
    ...
    while !form.is_web_ready() {
        ...
        if t0.elapsed() > Duration::from_secs(30) {
            eprintln!("webui-selftest: 前端 30s 未就绪 (WebView2 异常)");
            std::process::exit(3);
        }
    }
    println!(
        "webui-selftest: 前端就绪 (首启预热 {} ms)",
    ...
    let code = if let Some(n) = pick("--poc-showhide") {
        ...
        eprintln!(
            "用法: webui-selftest --poc-showhide <N> | --bench-reopen <N> | --poc-run <secs>"
        );
        1
    };
...
fn poc_showhide(form: &mut ShellForm, n: u64) -> i32 {
    for i in 0..n {
        ...
        if (i + 1) % 100 == 0 {
            println!("webui-selftest: showhide 进度 {}/{}", i + 1, n);
        }
    }
    println!("webui-selftest: showhide ×{n} 完成, 无 panic/退出请求");
    0
}
...
fn bench_reopen(form: &mut ShellForm, n: u64) -> i32 {
    ...
    for i in 0..n {
        ...
        if !ok {
            ...
            eprintln!(
                "webui-selftest: bench-reopen 第 {}/{} 轮回执 5s 超时 — FAIL (visible={vis:?}, web_ready={})",
                i + 1,
                n,
                form.is_web_ready()
            );
    ...
    println!(
        "webui-selftest: bench-reopen ×{n}  均值 {mean:.0} ms  P95 {p95} ms  最大 {} ms",
...
fn poc_run(form: &mut ShellForm, secs: u64) -> i32 {
    ...
    println!("webui-selftest: poc-run {secs}s 完成");
    0
}
