...
/// FMListRowRenderer); 对比/功率曲线窗口的机型下拉走本命令 (直连 kernel,
...
pub async fn fm_list() -> Result<serde_json::Value, String> {
    to_json(&kernel::fm::data_paths::list_fm_names("fm"))
}

/// 真机数据根注入 (数据域测试共用: data/ 缺失 → false, 调用方打印真因后 SKIP)
#[cfg(test)]
pub(crate) fn ensure_real_data() -> bool {
    // webui 位于 crates/webui → 仓库根 = ../.. (realtests 同款)
...
mod tests {
    ...
    fn 真机_fm列表_物理文件目录() {
        ...
        let planes = kernel::fm::data_paths::list_fm_names("fm");
