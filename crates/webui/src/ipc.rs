...
use kernel::formula::FormulaManager;
...
pub enum RequestKind {
    ...
    /// 表单消息 (→ ui main_form::update 全链, WYSIWYG 写回; 含 Save/StartGame 等)
...
/// 写点在 voidmei: 启动桥注入 load 过的实例 / 会话 start 覆盖 Service 实例。
...
/// dispatch 运行时 (阶段①壳态; 阶段②由 voidmei 注入 shell+form 真实现)。
...
pub struct FormRuntime {
    ...
    /// 公式编辑器直算面的共享 manager (缺省 = 出厂空集, voidmei 装配方注入真实例)
...
/// 由 voidmei 注入的 dispatcher 承担 — 本默认实现返回 Err (selftest 壳形态)。
...
mod tests {
    ...
    fn 数据面请求_壳形态拒绝() {
        ...
        // 而非静默成功 (voidmei 注入的真 dispatcher 承担; 协议转移自原 ui lib hooks 测试)
        let mut rt = FormRuntime::default();
        let cases = vec![
            RequestKind::GetLayoutTree,
            RequestKind::GetComboOptions {
        ...
        for c in cases {
            match dispatch(c, &mut rt) {
                IpcReply::Err(e) => assert!(e.contains("voidmei"), "拒绝信息应指向注入方: {e}"),
