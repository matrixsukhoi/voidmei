//! webui lib 入口 (D9): Tauri 2 MainForm 壳, 窗口常驻隐藏预热 + 主线程手动泵。
//!
//! 与 D8 拓扑的接法 (voidmei 主循环):
//! ```text
...
//! - UI_READY 语义: 每次 show 由 voidmei 主循环发布 (rebuild 后 Init→preview 保真),
//!   本 crate 不持有 ui_bus (依赖方向: voidmei → webui)。
//!
//! dispatcher 注入: 阶段①默认 [`ipc::dispatch`] (壳态); 阶段②起 voidmei 注入
...
/// dispatch 执行体签名 (主线程调用; 阶段②由 voidmei 注入 shell 写链实现)
...
impl ShellForm {
    ...
    /// `formula`: 公式编辑器直算面的共享 cell (E11 — 由调用方 (voidmei AppShell)
    ...
    pub fn new(dispatcher: Dispatcher, formula: FormulaShared) -> Result<Self, String> {
        ...
        let app = tauri::Builder::default()
            ...
            .invoke_handler(tauri::generate_handler![
                ...
                // P6 web 窗口数据域 (对比/功率曲线/机型选择; 直连 kernel,
